def train(hyp, opt, device, callbacks):
    (save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg,
        resume, noval, nosave, workers, freeze, mask_ratio) = (Path(opt.
        save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls,
        opt.evolve, opt.data, opt.cfg, opt.resume, opt.noval, opt.nosave,
        opt.workers, opt.freeze, opt.mask_ratio)
    w = save_dir / 'weights'
    ...
    names = {(0): 'item'} if single_cls and len(data_dict['names']
    ...
    if pretrained:
        if resume:
            best_fitness, start_epoch, epochs = smart_resume(ckpt,
                optimizer, ema, weights, epochs, resume)
        del ckpt, csd
...
See Multi-GPU Tutorial at https://docs.ultralytics.com/yolov5/tutorials/multi_gpu_training to get started."""
    ...
    train_loader, dataset = create_dataloader(train_path, imgsz, batch_size //
        ...
        prefix=colorstr('train: '), shuffle=True, mask_downsample_ratio=
        mask_ratio, overlap_mask=overlap)
    ...
    if RANK in {-1, 0}:
        val_loader = create_dataloader(val_path, imgsz, batch_size //
            ...
            mask_downsample_ratio=mask_ratio, overlap_mask=overlap, prefix=
            colorstr('val: '))[0]
        if not resume:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:
                results, maps, _ = validate.run(data_dict, batch_size=
                    batch_size // WORLD_SIZE * 2, imgsz=imgsz, half=amp,
                    model=ema.ema, single_cls=single_cls, dataloader=
                    val_loader, save_dir=save_dir, plots=False, callbacks=
                    callbacks, compute_loss=compute_loss,
                    mask_downsample_ratio=mask_ratio, overlap=overlap)
            ...
            if not nosave or final_epoch and not evolve:
                ckpt = {'epoch': epoch, 'best_fitness': best_fitness,
                    ...
                    'git': GIT_INFO, 'date': datetime.now().isoformat()}
...
{epoch - start_epoch + 1} epochs completed in {(time.time() - t0) / 3600:.3f} hours."""
        ...
        for f in (last, best):
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = validate.run(data_dict, batch_size=
                        ...
                        compute_loss=compute_loss, mask_downsample_ratio=
                        mask_ratio, overlap=overlap)
                    if is_coco:
                        metrics_dict = dict(zip(KEYS, list(mloss) + list(
                            results) + lr))
                        logger.log_metrics(metrics_dict, epoch)
        ...
        if plots:
            ...
            logger.log_images(sorted(save_dir.glob('val*.jpg')),
                'Validation', epoch + 1)
