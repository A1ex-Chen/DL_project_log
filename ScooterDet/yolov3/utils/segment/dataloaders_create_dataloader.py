def create_dataloader(path, imgsz, batch_size, stride, single_cls=False,
    ...
    mask_downsample_ratio=1, overlap_mask=False, seed=0):
    if rect and shuffle:
        LOGGER.warning(
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabelsAndMasks(path, imgsz, batch_size,
            augment=augment, hyp=hyp, rect=rect, cache_images=cache,
            single_cls=single_cls, stride=int(stride), pad=pad,
            image_weights=image_weights, prefix=prefix, downsample_ratio=
            mask_downsample_ratio, overlap=overlap_mask)
    ...
    return loader(dataset, batch_size=batch_size, shuffle=shuffle and 
        sampler is None, num_workers=nw, sampler=sampler, pin_memory=True,
        collate_fn=LoadImagesAndLabelsAndMasks.collate_fn4 if quad else
        LoadImagesAndLabelsAndMasks.collate_fn, worker_init_fn=seed_worker,
