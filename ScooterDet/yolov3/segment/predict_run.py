...
def run(weights=ROOT / 'yolov5s-seg.pt', source=ROOT / 'data/images', data=
    ROOT / 'data/coco128.yaml', imgsz=(640, 640), conf_thres=0.25,
    ...
    project=ROOT / 'runs/predict-seg', name='exp', exist_ok=False,
    line_thickness=3, hide_labels=False, hide_conf=False, half=False, dnn=
    False, vid_stride=1, retina_masks=False):
    source = str(source)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):
            ...
            if len(det):
                if retina_masks:
                    det[:, :4] = scale_boxes(im.shape[2:], det[:, :4], im0.
                        shape).round()
                    masks = process_mask_native(proto[i], det[:, 6:], det[:,
                        :4], im0.shape[:2])
                else:
                    masks = process_mask(proto[i], det[:, 6:], det[:, :4],
                        im.shape[2:], upsample=True)
                    det[:, :4] = scale_boxes(im.shape[2:], det[:, :4], im0.
                        shape).round()
                if save_txt:
                    segments = [scale_segments(im0.shape if retina_masks else
                        im.shape[2:], x, im0.shape, normalize=True) for x in
                        reversed(masks2segments(masks))]
                for c in det[:, 5].unique():
                    n = (det[:, 5] == c).sum()
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "
                annotator.masks(masks, colors=[colors(x, True) for x in det
                    [:, 5]], im_gpu=torch.as_tensor(im0, dtype=torch.
                    float16).to(device).permute(2, 0, 1).flip(0).contiguous
