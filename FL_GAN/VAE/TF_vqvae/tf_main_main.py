def main():
    ...
    vqvae_metric_save_path = (iwantto_path +
        f"/{args.dataset}/v1/{'dp' if args.dpsgd else 'normal'}/metric")
    ...
    reconstruction_path_traditional_flow = (iwantto_path +
        f"/{args.dataset}/v1/{'dp' if args.dpsgd else 'normal'}/Images")
    ...
    show_latent(test_images[:args.latent_num], codebook_indices[:args.
        latent_num], tf.cast(reconstruction_images[:args.latent_num] * 255,
        tf.int32), latent_save_path, False if test_images.shape[3] == 3 else
        True)
    """
    ...
    for i in tqdm.tqdm(range(int(flat_enc_outputs.shape[0] / 1000))):
        codebook_indices = np.concatenate((codebook_indices, quantizer.
            get_code_indices(flat_enc_outputs[i * 1000:(i + 1) * 1000]).
            numpy()), axis=0)
    ...
    pixel_history = pixel_cnn.fit(x=codebook_indices, y=codebook_indices,
        batch_size=args.batch_size, epochs=args.epochs, validation_split=
        0.1, callbacks=[Pixel_CustomCallback()])
    ...
    pixel_cnn_metric_save_path = (pixel_cnn_save_path +
        f'/pixel_cnn_metrics_{args.epochs}.csv')
