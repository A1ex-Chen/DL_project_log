...
def __call__(self, image: Union[PIL.Image.Image, np.ndarray, torch.Tensor],
    original_image: Union[PIL.Image.Image, torch.Tensor, np.ndarray, List[
    PIL.Image.Image], List[torch.Tensor], List[np.ndarray]]=None,
    mask_image: Union[PIL.Image.Image, torch.Tensor, np.ndarray, List[PIL.
    Image.Image], List[torch.Tensor], List[np.ndarray]]=None, strength:
    float=0.8, prompt: Union[str, List[str]]=None, num_inference_steps: int
    =100, timesteps: List[int]=None, guidance_scale: float=4.0,
    ...
    Optional[Dict[str, Any]]=None, noise_level: int=0, clean_caption: bool=True
    ):
    """
    ...
    self.check_inputs(prompt, image, original_image, mask_image, batch_size,
    ...
    intermediate_images = self.prepare_intermediate_images(original_image,
    ...
    upscaled = F.interpolate(image, (height, width), mode='bilinear',
        align_corners=True)
    noise_level = torch.tensor([noise_level] * upscaled.shape[0], device=
        upscaled.device)
    noise = randn_tensor(upscaled.shape, generator=generator, device=
        upscaled.device, dtype=upscaled.dtype)
    upscaled = self.image_noising_scheduler.add_noise(upscaled, noise,
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            noise_pred = self.unet(model_input, t, encoder_hidden_states=
                prompt_embeds, class_labels=noise_level,
            ...
            if self.scheduler.config.variance_type not in ['learned',
                'learned_range']:
                noise_pred, _ = noise_pred.split(intermediate_images.shape[
                    1], dim=1)
