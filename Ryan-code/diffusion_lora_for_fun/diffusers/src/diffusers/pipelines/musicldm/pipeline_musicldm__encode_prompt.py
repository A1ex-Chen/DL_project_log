def _encode_prompt(self, prompt, device, num_waveforms_per_prompt,
    ...
    if prompt_embeds is None:
        ...
        prompt_embeds = self.text_encoder.get_text_features(text_input_ids.
            to(device), attention_mask=attention_mask.to(device))
    prompt_embeds = prompt_embeds.to(dtype=self.text_encoder.text_model.
        dtype, device=device)
    ...
    if do_classifier_free_guidance:
        ...
        negative_prompt_embeds = negative_prompt_embeds.to(dtype=self.
            text_encoder.text_model.dtype, device=device)
