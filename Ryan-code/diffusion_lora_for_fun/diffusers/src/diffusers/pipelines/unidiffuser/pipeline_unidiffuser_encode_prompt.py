def encode_prompt(self, prompt, device, num_images_per_prompt,
    ...
    if prompt_embeds is None:
        ...
        text_inputs = self.clip_tokenizer(prompt, padding='max_length',
            max_length=self.clip_tokenizer.model_max_length, truncation=
            True, return_tensors='pt')
        text_input_ids = text_inputs.input_ids
        untruncated_ids = self.clip_tokenizer(prompt, padding='longest',
            return_tensors='pt').input_ids
        if untruncated_ids.shape[-1] >= text_input_ids.shape[-1
            ] and not torch.equal(text_input_ids, untruncated_ids):
            removed_text = self.clip_tokenizer.batch_decode(untruncated_ids
                [:, self.clip_tokenizer.model_max_length - 1:-1])
            logger.warning(
    ...
    if do_classifier_free_guidance and negative_prompt_embeds is None:
        ...
        uncond_input = self.clip_tokenizer(uncond_tokens, padding=
