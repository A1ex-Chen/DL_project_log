...
def from_single_file(cls, pretrained_model_link_or_path_or_dict: Optional[
    str]=None, **kwargs):
    """
    ...
    pretrained_model_link_or_path = kwargs.get('pretrained_model_link_or_path',
        None)
    ...
    if isinstance(pretrained_model_link_or_path_or_dict, dict):
        ...
        checkpoint = load_single_file_checkpoint(
            pretrained_model_link_or_path_or_dict, resume_download=
            resume_download, force_download=force_download, proxies=proxies,
            token=token, cache_dir=cache_dir, local_files_only=
    ...
    if original_config:
        ...
        if config_mapping_fn is None:
            raise ValueError(
                f'`original_config` has been provided for {class_name} but no mapping functionwas found to convert the original config to a Diffusers config in`diffusers.loaders.single_file_utils`'
                )
        if isinstance(original_config, str):
            original_config = fetch_original_config(original_config,
                local_files_only=local_files_only)
        config_mapping_kwargs = _get_mapping_function_kwargs(config_mapping_fn,
            **kwargs)
        diffusers_model_config = config_mapping_fn(original_config=
            original_config, checkpoint=checkpoint, **config_mapping_kwargs)
    else:
        if config:
            if isinstance(config, str):
                ...
                raise ValueError(
                    'Invalid `config` argument. Please provide a string representing a repo idor path to a local Diffusers model repo.'
                    )
        else:
        ...
        diffusers_model_config = cls.load_config(pretrained_model_name_or_path
            =default_pretrained_model_config_name, subfolder=subfolder,
            local_files_only=local_files_only)
        ...
        model_kwargs = {k: kwargs.get(k) for k in kwargs if k in
            expected_kwargs or k in optional_kwargs}
        diffusers_model_config.update(model_kwargs)
    checkpoint_mapping_kwargs = _get_mapping_function_kwargs(
        checkpoint_mapping_fn, **kwargs)
    diffusers_format_checkpoint = checkpoint_mapping_fn(config=
        diffusers_model_config, checkpoint=checkpoint, **
