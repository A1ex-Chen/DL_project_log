def main(args):
    ...
    def preprocess_train(examples):
        ...
        for col_name in ['jpg_0', 'jpg_1']:
            ...
            if col_name == 'jpg_1':
                images = [image.resize(original_sizes[i][::-1]) for i,
        ...
        for im_tup, label_0 in zip(im_tup_iterator, examples['label_0']):
            ...
            if not args.random_crop:
                y1 = max(0, int(round((combined_im.shape[1] - args.
                    resolution) / 2.0)))
                x1 = max(0, int(round((combined_im.shape[2] - args.
                    resolution) / 2.0)))
            ...
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    ref_preds = unet(noisy_model_input, timesteps,
                        prompt_embeds, added_cond_kwargs={'time_ids':
                        add_time_ids, 'text_embeds': pooled_prompt_embeds}
                        ).sample
                ...
                implicit_acc += 0.5 * (inside_term == 0).sum().float(
                    ) / inside_term.size(0)
