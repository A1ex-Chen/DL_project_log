def log_validation(args, unet, accelerator, weight_dtype, epoch,
    is_final_validation=False):
    logger.info(
        f'Running validation... \n Generating images with prompts:\n {VALIDATION_PROMPTS}.'
        )
    pipeline = DiffusionPipeline.from_pretrained(args.
        pretrained_model_name_or_path, revision=args.revision, variant=args
        .variant, torch_dtype=weight_dtype)
    ...
    for prompt in VALIDATION_PROMPTS:
        with context:
            image = pipeline(prompt, num_inference_steps=25, generator=
                generator).images[0]
            images.append(image)
    ...
    if is_final_validation:
        ...
        no_lora_images = [pipeline(prompt, num_inference_steps=25,
            generator=generator).images[0] for prompt in VALIDATION_PROMPTS]
        for tracker in accelerator.trackers:
            if tracker.name == 'tensorboard':
