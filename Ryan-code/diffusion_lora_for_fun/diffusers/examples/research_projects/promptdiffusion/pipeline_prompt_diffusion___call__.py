...
def __call__(self, prompt: Union[str, List[str]]=None, image:
    PipelineImageInput=None, image_pair: List[PipelineImageInput]=None,
    ...
    self.check_inputs(prompt, image, image_pair, callback_steps,
    ...
    if isinstance(controlnet, ControlNetModel):
        image_pair = torch.cat([self.prepare_image(image=im, width=width,
            height=height, batch_size=batch_size * num_images_per_prompt,
            num_images_per_prompt=num_images_per_prompt, device=device,
            dtype=controlnet.dtype, do_classifier_free_guidance=self.
            do_classifier_free_guidance, guess_mode=guess_mode) for im in
            image_pair], 1)
    if isinstance(controlnet, ControlNetModel):
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                ...
                controlnet_prompt_embeds, controlnet_query_cond=image,
                controlnet_cond=image_pair, conditioning_scale=cond_scale,
