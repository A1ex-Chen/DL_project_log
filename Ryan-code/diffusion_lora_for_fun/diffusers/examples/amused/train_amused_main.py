def main(args):
    ...
    vq_model = VQModel.from_pretrained(args.pretrained_model_name_or_path,
        subfolder='vqvae', revision=args.revision, variant=args.variant)
    if args.train_text_encoder:
        if args.text_encoder_use_lora:
            lora_config = LoraConfig(r=args.text_encoder_lora_r, lora_alpha
                =args.text_encoder_lora_alpha, target_modules=args.
    ...
    model = UVit2DModel.from_pretrained(args.pretrained_model_name_or_path,
    ...
    if args.use_ema:
        ema = EMAModel(model.parameters(), decay=args.ema_decay,
            update_after_step=args.ema_update_after_step, model_cls=
            UVit2DModel, model_config=model.config)
        ...
        if accelerator.is_main_process:
            ...
            for model_ in models:
                if isinstance(model_, type(accelerator.unwrap_model(model))):
                    ...
                    raise ValueError(
                        f'unexpected save model: {model_.__class__}')
                weights.pop()
    ...
    no_decay = ['bias', 'layer_norm.weight', 'mlm_ln.weight',
        'embeddings.weight']
    optimizer_grouped_parameters = [{'params': [p for n, p in model.
        named_parameters() if not any(nd in n for nd in no_decay)],
        'weight_decay': args.adam_weight_decay}, {'params': [p for n, p in
        model.named_parameters() if any(nd in n for nd in no_decay)],
        'weight_decay': 0.0}]
    if args.train_text_encoder:
        optimizer_grouped_parameters.append({'params': text_encoder.
            parameters(), 'weight_decay': args.adam_weight_decay})
    optimizer = optimizer_cls(optimizer_grouped_parameters, lr=args.
    ...
    if args.instance_data_dir is not None:
        dataset = InstanceDataRootDataset(instance_data_root=args.
            instance_data_dir, tokenizer=tokenizer, size=args.resolution)
    elif args.instance_data_image is not None:
        dataset = InstanceDataImageDataset(instance_data_image=args.
            instance_data_image, train_batch_size=args.train_batch_size,
            size=args.resolution)
    elif args.instance_data_dataset is not None:
        dataset = HuggingFaceDataset(hf_dataset=load_dataset(args.
            instance_data_dataset, split='train'), tokenizer=tokenizer,
            image_key=args.image_key, prompt_key=args.prompt_key,
            prompt_prefix=args.prompt_prefix, size=args.resolution)
    else:
        assert False
    train_dataloader = DataLoader(dataset, batch_size=args.train_batch_size,
        shuffle=True, num_workers=args.dataloader_num_workers, collate_fn=
        default_collate)
    train_dataloader.num_batches = len(train_dataloader)
    lr_scheduler = diffusers.optimization.get_scheduler(args.lr_scheduler,
        optimizer=optimizer, num_training_steps=args.max_train_steps *
        accelerator.num_processes, num_warmup_steps=args.lr_warmup_steps *
    ...
    with (nullcontext() if args.train_text_encoder else torch.no_grad()):
        empty_embeds, empty_clip_embeds = encode_prompt(text_encoder,
            tokenize_prompt(tokenizer, '').to(text_encoder.device,
            non_blocking=True))
        if args.instance_data_image is not None:
            prompt = os.path.splitext(os.path.basename(args.
                instance_data_image))[0]
            encoder_hidden_states, cond_embeds = encode_prompt(text_encoder,
                tokenize_prompt(tokenizer, prompt).to(text_encoder.device,
                non_blocking=True))
            encoder_hidden_states = encoder_hidden_states.repeat(args.
    ...
    if resume_from_checkpoint is None:
        ...
        global_step = int(os.path.basename(resume_from_checkpoint).split(
            '-')[1])
        first_epoch = global_step // num_update_steps_per_epoch
    for epoch in range(first_epoch, num_train_epochs):
        for batch in train_dataloader:
            with torch.no_grad():
                micro_conds = batch['micro_conds'].to(accelerator.device,
                    non_blocking=True)
                pixel_values = batch['image'].to(accelerator.device,
                    non_blocking=True)
                batch_size = pixel_values.shape[0]
                split_batch_size = (args.split_vae_encode if args.
                    split_vae_encode is not None else batch_size)
                ...
                for i in range(num_splits):
                    ...
                    image_tokens.append(vq_model.quantize(vq_model.encode(
                        pixel_values[start_idx:end_idx]).latents)[2][2].
                        reshape(bs, -1))
                ...
                batch_randperm = torch.rand(batch_size, seq_len, device=
                    image_tokens.device).argsort(dim=-1)
                ...
                if args.cond_dropout_prob > 0.0:
                    ...
                    mask = torch.zeros((batch_size, 1, 1), device=
                        encoder_hidden_states.device).float().uniform_(0, 1
                        ) < args.cond_dropout_prob
                    empty_embeds_ = empty_embeds.expand(batch_size, -1, -1)
                    encoder_hidden_states = torch.where((
                        encoder_hidden_states * mask).bool(),
                        encoder_hidden_states, empty_embeds_)
                    empty_clip_embeds_ = empty_clip_embeds.expand(batch_size,
                        -1)
                    cond_embeds = torch.where((cond_embeds * mask.squeeze(-
                        1)).bool(), cond_embeds, empty_clip_embeds_)
                bs = input_ids.shape[0]
                vae_scale_factor = 2 ** (len(vq_model.config.
                    block_out_channels) - 1)
                resolution = args.resolution // vae_scale_factor
                input_ids = input_ids.reshape(bs, resolution, resolution)
            if 'prompt_input_ids' in batch:
                with (nullcontext() if args.train_text_encoder else torch.
                    no_grad()):
                    encoder_hidden_states, cond_embeds = encode_prompt(
                        text_encoder, batch['prompt_input_ids'].to(
                        accelerator.device, non_blocking=True))
            with accelerator.accumulate(model):
                codebook_size = accelerator.unwrap_model(model
                    ).config.codebook_size
                logits = model(input_ids=input_ids, encoder_hidden_states=
                    encoder_hidden_states, micro_conds=micro_conds,
                    pooled_text_emb=cond_embeds).reshape(bs, codebook_size, -1
                    ).permute(0, 2, 1).reshape(-1, codebook_size)
                loss = F.cross_entropy(logits, labels.view(-1),
                    ignore_index=-100, reduction='mean')
                avg_loss = accelerator.gather(loss.repeat(args.
                    train_batch_size)).mean()
                avg_masking_rate = accelerator.gather(mask_prob.repeat(args
                    .train_batch_size)).mean()
                accelerator.backward(loss)
                if (args.max_grad_norm is not None and accelerator.
                    sync_gradients):
                    accelerator.clip_grad_norm_(model.parameters(), args.
                        max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step + 1) % args.logging_steps == 0:
                    logs = {'step_loss': avg_loss.item(), 'lr':
                        lr_scheduler.get_last_lr()[0], 'avg_masking_rate':
                        avg_masking_rate.item()}
                    accelerator.log(logs, step=global_step + 1)
                    logger.info(
                        f'Step: {global_step + 1} Loss: {avg_loss.item():0.4f} LR: {lr_scheduler.get_last_lr()[0]:0.6f}'
                        )
                ...
                if (global_step + 1
                    ...
                    with torch.no_grad():
                        ...
                        if args.train_text_encoder:
                            text_encoder.eval()
                        scheduler = AmusedScheduler.from_pretrained(args.
                            pretrained_model_name_or_path, subfolder=
                            'scheduler', revision=args.revision, variant=
                            args.variant)
                        pipe = AmusedPipeline(transformer=accelerator.
                            unwrap_model(model), tokenizer=tokenizer,
                            text_encoder=text_encoder, vqvae=vq_model,
                            scheduler=scheduler)
                        pil_images = pipe(prompt=args.validation_prompts
                            ).images
                        wandb_images = [wandb.Image(image, caption=args.
                            validation_prompts[i]) for i, image in
                            enumerate(pil_images)]
                        wandb.log({'generated_images': wandb_images}, step=
                            global_step + 1)
                        model.train()
                        if args.train_text_encoder:
                            text_encoder.train()
