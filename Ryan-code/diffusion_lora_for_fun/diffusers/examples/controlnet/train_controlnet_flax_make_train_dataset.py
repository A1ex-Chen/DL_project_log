def make_train_dataset(args, tokenizer, batch_size=None):
    if args.dataset_name is not None:
        ...
        if args.load_from_disk:
            ...
            dataset = load_dataset(args.train_data_dir, cache_dir=args.
                cache_dir)
    ...
    if jax.process_index() == 0:
        ...
        if args.streaming:
            train_dataset = dataset['train'].map(preprocess_train, batched=
                True, batch_size=batch_size, remove_columns=list(dataset[
                'train'].features.keys()))
        else:
