...
def __call__(self, prompt: Union[str, List[str]], height: int=512, width:
    ...
    prompt_language = detect_language(self.detection_pipeline, prompt,
        batch_size)
    if batch_size == 1 and prompt_language != 'en':
        prompt = translate_prompt(prompt, self.translation_tokenizer, self.
            translation_model, self.device)
    if isinstance(prompt, list):
        for index in range(batch_size):
            if prompt_language[index] != 'en':
                p = translate_prompt(prompt[index], self.
                    translation_tokenizer, self.translation_model, self.device)
                prompt[index] = p
    ...
    if do_classifier_free_guidance:
        ...
        if negative_prompt is None:
            ...
            negative_prompt_language = detect_language(self.
                detection_pipeline, negative_prompt, batch_size)
            if negative_prompt_language != 'en':
                negative_prompt = translate_prompt(negative_prompt, self.
                    translation_tokenizer, self.translation_model, self.device)
            ...
            if isinstance(negative_prompt, list):
                negative_prompt_languages = detect_language(self.
                    detection_pipeline, negative_prompt, batch_size)
                for index in range(batch_size):
                    if negative_prompt_languages[index] != 'en':
                        p = translate_prompt(negative_prompt[index], self.
                            translation_tokenizer, self.translation_model,
                            self.device)
