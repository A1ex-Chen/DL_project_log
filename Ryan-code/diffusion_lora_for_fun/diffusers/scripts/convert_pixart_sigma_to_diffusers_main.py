def main(args):
    ...
    for depth in range(28):
        ...
        if args.qk_norm:
            converted_state_dict[
                f'transformer_blocks.{depth}.attn1.q_norm.weight'
                ] = state_dict.pop(f'blocks.{depth}.attn.q_norm.weight')
            converted_state_dict[
                f'transformer_blocks.{depth}.attn1.q_norm.bias'
                ] = state_dict.pop(f'blocks.{depth}.attn.q_norm.bias')
            converted_state_dict[
                f'transformer_blocks.{depth}.attn1.k_norm.weight'
                ] = state_dict.pop(f'blocks.{depth}.attn.k_norm.weight')
            converted_state_dict[
                f'transformer_blocks.{depth}.attn1.k_norm.bias'
    ...
    transformer = Transformer2DModel(sample_size=args.image_size // 8,
        ...
        norm_elementwise_affine=False, norm_eps=1e-06, caption_channels=
        4096, interpolation_scale=interpolation_scale[args.image_size],
    ...
    if args.only_transformer:
        ...
        tokenizer = T5Tokenizer.from_pretrained(
            f'{ckpt_id}/pixart_sigma_sdxlvae_T5_diffusers', subfolder=
            'tokenizer')
        text_encoder = T5EncoderModel.from_pretrained(
            f'{ckpt_id}/pixart_sigma_sdxlvae_T5_diffusers', subfolder=
            'text_encoder')
        pipeline = PixArtSigmaPipeline(tokenizer=tokenizer, text_encoder=
