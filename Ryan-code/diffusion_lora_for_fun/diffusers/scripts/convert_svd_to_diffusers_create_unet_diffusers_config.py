def create_unet_diffusers_config(original_config, image_size: int,
    ...
    for i in range(len(block_out_channels)):
        block_type = ('CrossAttnDownBlockSpatioTemporal' if resolution in
            unet_params.attention_resolutions else 'DownBlockSpatioTemporal')
        down_block_types.append(block_type)
        if i != len(block_out_channels) - 1:
            resolution *= 2
    up_block_types = []
    for i in range(len(block_out_channels)):
        block_type = ('CrossAttnUpBlockSpatioTemporal' if resolution in
            unet_params.attention_resolutions else 'UpBlockSpatioTemporal')
        up_block_types.append(block_type)
        resolution //= 2
    if unet_params.transformer_depth is not None:
        transformer_layers_per_block = (unet_params.transformer_depth if
            isinstance(unet_params.transformer_depth, int) else list(
