def convert_ldm_unet_checkpoint(checkpoint, config, path=None, extract_ema=
    ...
    new_checkpoint['time_embedding.linear_2.bias'] = unet_state_dict[
        'time_embed.2.bias']
    additional_embedding_substrings = ['local_image_concat',
        'context_embedding', 'local_image_embedding', 'fps_embedding']
    for k in unet_state_dict:
        if any(substring in k for substring in additional_embedding_substrings
            ):
            diffusers_key = k.replace('local_image_concat',
                'image_latents_proj_in').replace('local_image_embedding',
                'image_latents_context_embedding')
            new_checkpoint[diffusers_key] = unet_state_dict[k]
    new_checkpoint['image_latents_temporal_encoder.norm1.weight'
        ] = unet_state_dict['local_temporal_encoder.layers.0.0.norm.weight']
    new_checkpoint['image_latents_temporal_encoder.norm1.bias'
        ] = unet_state_dict['local_temporal_encoder.layers.0.0.norm.bias']
    ...
    new_checkpoint['image_latents_temporal_encoder.attn1.to_out.0.weight'
        ] = unet_state_dict[
        'local_temporal_encoder.layers.0.0.fn.to_out.0.weight']
    new_checkpoint['image_latents_temporal_encoder.attn1.to_out.0.bias'
        ] = unet_state_dict[
        'local_temporal_encoder.layers.0.0.fn.to_out.0.bias']
    new_checkpoint['image_latents_temporal_encoder.ff.net.0.proj.weight'
        ] = unet_state_dict['local_temporal_encoder.layers.0.1.net.0.0.weight']
    new_checkpoint['image_latents_temporal_encoder.ff.net.0.proj.bias'
        ] = unet_state_dict['local_temporal_encoder.layers.0.1.net.0.0.bias']
    new_checkpoint['image_latents_temporal_encoder.ff.net.2.weight'
        ] = unet_state_dict['local_temporal_encoder.layers.0.1.net.2.weight']
    new_checkpoint['image_latents_temporal_encoder.ff.net.2.bias'
        ] = unet_state_dict['local_temporal_encoder.layers.0.1.net.2.bias']
    if 'class_embed_type' in config:
        if config['class_embed_type'] is None:
            ...
        elif config['class_embed_type'] == 'timestep' or config[
            'class_embed_type'] == 'projection':
            new_checkpoint['class_embedding.linear_1.weight'
                ] = unet_state_dict['label_emb.0.0.weight']
            new_checkpoint['class_embedding.linear_1.bias'] = unet_state_dict[
                'label_emb.0.0.bias']
            new_checkpoint['class_embedding.linear_2.weight'
                ] = unet_state_dict['label_emb.0.2.weight']
