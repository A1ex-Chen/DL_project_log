...
def log(request):
    if request.method == 'POST':
        ...
        user_id = pd.DataFrame.from_records(user_id, columns=['id']).values[0][
            0]
        ...
        l = Rating(user_id=user_id, item_id=str(item_id), rating=rating,
            rating_timestamp=date, type=type)
        l.save()
    else:
