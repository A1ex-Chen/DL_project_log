...
def predict(self, model: Model) ->Prediction:
    ...
    for _, item_batch in enumerate(tqdm(self._dataloader, mininterval=10)):
        ...
        (_, _, _, detection_bboxes_batch, detection_classes_batch,
            detection_probs_batch) = model.eval().forward(processed_image_batch
            )
    ...
    return Evaluator.Prediction(sorted_all_image_ids,
        ...
        sorted_all_pred_probs, image_id_to_pred_bboxes_dict,
        image_id_to_pred_classes_dict, image_id_to_pred_probs_dict,
        image_id_to_gt_bboxes_dict, image_id_to_gt_classes_dict,
