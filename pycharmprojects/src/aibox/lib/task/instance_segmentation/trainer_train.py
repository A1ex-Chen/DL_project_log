    ...
    for epoch in self._epoch_range:
        ...
        for batch_index, item_tuple_batch in enumerate(self._dataloader):
            ...
            processed_bboxes_batch = Bunch([it.processed_bboxes for it in
                item_batch])
            processed_masks_batch = Bunch([it.processed_masks for it in
                item_batch])
            classes_batch = Bunch([it.classes for it in item_batch])
            (anchor_objectness_losses, anchor_transformer_losses,
                proposal_class_losses, proposal_transformer_losses, mask_losses
                ) = (self._model.forward(processed_image_batch,
                processed_bboxes_batch, processed_masks_batch, classes_batch))
            anchor_objectness_loss = torch.stack(anchor_objectness_losses,
                dim=0).mean()
            anchor_transformer_loss = torch.stack(anchor_transformer_losses,
                dim=0).mean()
            proposal_class_loss = torch.stack(proposal_class_losses, dim=0
                ).mean()
            proposal_transformer_loss = torch.stack(proposal_transformer_losses
                , dim=0).mean()
            mask_loss = torch.stack(mask_losses, dim=0).mean()
            loss = (anchor_objectness_loss + anchor_transformer_loss +
                proposal_class_loss + proposal_transformer_loss + mask_loss)
            ...
            self._callback.on_batch_end(epoch, num_batches_in_epoch,
                ...
                proposal_transformer_loss.item(), mask_loss.item())
