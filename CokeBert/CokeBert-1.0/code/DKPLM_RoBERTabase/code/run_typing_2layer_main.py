...
"""
    ...
    model, _ = BertForEntityTyping.from_pretrained(args.ernie_model,
        ...
        args.local_rank), num_labels=len(label_list), args=args)
    if args.fp16:
        model.half()
    model.to(device)
    ...
    if args.do_train:
        ...
        all_ent_mask = torch.tensor([f.ent_mask for f in train_features],
            dtype=torch.long)
        all_labels = torch.tensor([f.labels for f in train_features], dtype
            =torch.float)
        ...
        for epoch in trange(int(args.num_train_epochs), desc='Epoch'):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc=
                ...
                loss, logits = model(input_ids, segment_ids, input_mask,
                    input_ent.float(), ent_mask, labels.half(), k_1.half(),
                    v_1.half(), k_2.half(), v_2.half())
            ...
            output_model_file = os.path.join(args.output_dir,
                'pytorch_model.bin_{}'.format(epoch))
