...
>> transformers xlm XLM_CHECKPOINT_PATH PYTORCH_DUMP_OUTPUT"""
        ...
        try:
            ...
            print(
                'transformers can only be used from the commandline to convert TensorFlow models in PyTorch, In that case, it requires TensorFlow to be installed. Please see https://www.tensorflow.org/install/ for installation instructions.'
                )
            raise
        if len(sys.argv) != 5:
            print(
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            ...
            convert_openai_checkpoint_to_pytorch(
                OPENAI_GPT_CHECKPOINT_FOLDER_PATH, OPENAI_GPT_CONFIG,
                PYTORCH_DUMP_OUTPUT)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            ...
            convert_transfo_xl_checkpoint_to_pytorch(TF_CHECKPOINT,
                TF_CONFIG, PYTORCH_DUMP_OUTPUT, TF_DATASET_FILE)
        ...
        if len(sys.argv) < 4 or len(sys.argv) > 5:
            ...
            if len(sys.argv) == 5:
                ...
                TF_CONFIG = ''
            convert_gpt2_checkpoint_to_pytorch(TF_CHECKPOINT, TF_CONFIG,
                PYTORCH_DUMP_OUTPUT)
        ...
        if len(sys.argv) < 5 or len(sys.argv) > 6:
            ...
            convert_xlnet_checkpoint_to_pytorch(TF_CHECKPOINT, TF_CONFIG,
                PYTORCH_DUMP_OUTPUT, FINETUNING_TASK)
    elif sys.argv[1] == 'xlm':
