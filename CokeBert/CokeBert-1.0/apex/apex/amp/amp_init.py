def init(enabled=True, loss_scale='dynamic', enable_caching=True, verbose=
    False, allow_banned=False):
    ...
    cast_table = [('FP16_FUNCS', utils.maybe_half), ('FP32_FUNCS', utils.
        maybe_float)]
    ...
    promote_table = [('CASTS', wrap.promote), ('SEQUENCE_CASTS', wrap.
        sequence_promote)]
    ...
    if compat.tensor_is_float_tensor():
        for cls, (list_name, promote_fn) in itertools.product([torch.cuda.
            FloatTensor, torch.cuda.HalfTensor], promote_table):
    ...
    for fn in utils.as_inplace(itertools.chain(tensor_overrides.FP16_FUNCS,
    ...
    for fn, err_msg in functional_overrides.BANNED_FUNCS:
        if allow_banned:
            wrap.cached_cast(functional_overrides.MODULE, fn, utils.
                maybe_float, handle, try_caching=True, verbose=verbose)
        else:
            wrap.err_if_any_half(functional_overrides.MODULE, fn, handle,
                err_msg)
    _DECORATOR_HANDLE = handle
