...
def get_flowers102(data_root=None, img_size=224, batch_size=64,
    ...
    augmentation_mode='vanilla', train_transforms=None, val_transforms=None,
    mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, crop_pct=
    ...
    use_multi_epochs_loader=False, worker_seeding='all', **kwargs):
    ...
    dataset_train = Flowers102(root=data_root, split='train', download=
        download, transform=train_transforms or default_train_transforms)
    dataset_eval = Flowers102(root=data_root, split='test', download=
        download, transform=val_transforms or default_val_transforms)
