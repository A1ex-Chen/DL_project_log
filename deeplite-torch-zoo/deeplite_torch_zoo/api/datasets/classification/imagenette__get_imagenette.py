def _get_imagenette(data_root=None, img_size=224, batch_size=64,
    ...
    .device('cuda'), augmentation_mode='imagenet', train_transforms=None,
    val_transforms=None, mean=IMAGENET_DEFAULT_MEAN, std=
    ...
    dataset_train = Imagenette(root=data_root, split='train', download=
        download, transform=train_transforms or default_train_transforms,
        url=dataset_url)
    dataset_eval = Imagenette(root=data_root, split='val', download=
        download, transform=val_transforms or default_val_transforms, url=
        dataset_url)
