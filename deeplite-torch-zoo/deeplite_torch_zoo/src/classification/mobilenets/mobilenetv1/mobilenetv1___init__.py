def __init__(self, num_classes=2, width_mult=1.0, last_pooling_size=7):
    ...
    def conv_dw(inp, oup, stride):
        ...
        return nn.Sequential(nn.Conv2d(inp, inp, 3, stride, 1, groups=inp,
            bias=False), nn.BatchNorm2d(inp), nn.ReLU(inplace=True), nn.
            Conv2d(inp, oup, 1, 1, 0, bias=False), nn.BatchNorm2d(oup), nn.
            ReLU(inplace=True))
    self.model = nn.Sequential(conv_bn(3, 32, 2), conv_dw(32, 64, 1),
        ...
        conv_dw(512, 512, 1), conv_dw(512, 1024, 2), conv_dw(1024, 1024, 1),
