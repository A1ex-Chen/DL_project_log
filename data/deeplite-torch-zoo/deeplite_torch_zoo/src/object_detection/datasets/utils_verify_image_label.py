    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl:
                ...
                assert max_cls <= num_cls, f'Label class {max_cls} exceeds dataset class count {num_cls}. Possible class labels are 0-{num_cls - 1}'
                ...
                lb = np.zeros((0, 5 + nkpt * ndim), dtype=np.float32
                    ) if keypoint else np.zeros((0, 5), dtype=np.float32)
        else:
            nm = 1
            lb = np.zeros((0, 5 + nkpt * ndim), dtype=np.float32
                ) if keypoint else np.zeros((0, 5), dtype=np.float32)
