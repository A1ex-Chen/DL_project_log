...
def get_imagenet(data_root, img_size=224, batch_size=64, test_batch_size=
    None, train_split='training', val_split='validation', class_map=None,
    download=False, seed=42, repeats=0, use_prefetcher=False, num_workers=1,
    ...
    default_train_transforms, default_val_transforms = get_imagenet_transforms(
        img_size, mean=mean, std=std, crop_pct=crop_pct, scale=scale, ratio
        =ratio, hflip=hflip, vflip=vflip, color_jitter=color_jitter,
        auto_augment=auto_augment, train_interpolation=train_interpolation,
        test_interpolation=test_interpolation, re_prob=re_prob, re_mode=
        re_mode, re_count=re_count, re_num_splits=re_num_splits,
        use_prefetcher=use_prefetcher)
    dataset_train = create_dataset('imagenet', root=data_root, split=
        train_split, is_training=True, class_map=class_map, download=
        download, batch_size=batch_size, seed=seed, repeats=repeats)
    dataset_train.transform = train_transforms or default_train_transforms
    dataset_train.classes = range(1000)
    dataset_eval = create_dataset('imagenet', root=data_root, split=
        val_split, is_training=False, class_map=class_map, download=
        download, batch_size=batch_size)
