def main():
    ...
    args.output_dir = os.path.join(args.output_dir, 'pretrain_coke-' + args
    ...
    model = CokeBertForPreTraining.from_pretrained(os.path.join(
    ...
    def collate_fn(x):
        ...
        return x[:, :args.max_seq_length], x[:, args.max_seq_length:2 *
            ...
            max_seq_length], x[:, 3 * args.max_seq_length:4 * args.
            max_seq_length], ent_emb, mask, x[:, 6 * args.max_seq_length:
    ...
    if args.do_train:
        ...
        train_data = indexed_dataset.IndexedDataset(os.path.join(args.
            data_dir, args.backbone), fix_lua_indexing=True)
        ...
        no_linear = ['layer.2.output.dense_ent',
            'layer.2.intermediate.dense_1',
        ...
        for _ in trange(int(args.num_train_epochs), desc='Epoch'):
            ...
            for step, batch in enumerate(tqdm(train_iterator.next_epoch_itr
                (), desc='Iteration')):
                batch = tuple(t.to(device) for t in batch)
                (input_ids, input_mask, token_type_ids, mlm_labels,
                    input_ent, ent_mask, nsp_labels, ent_candidate, ent_labels
                    ) = batch
                input_ent += 1
                (k_1, v_1, k_2, v_2, k_cand_1, v_cand_1, k_cand_2, v_cand_2,
                    cand_pos_tensor) = (load_k_v_queryR_small(input_ent,
                    ent_candidate, ent_neighbor, ent_r, ent_outORin,
                    embed_ent, embed_r, args.neighbor_hop))
                cand_pos_tensor = cand_pos_tensor.to(device)
                loss, original_loss = model(input_ids, input_mask,
                    token_type_ids, labels=mlm_labels, input_ent=input_ent,
                    ent_mask=ent_mask, next_sentence_label=nsp_labels,
                    candidate=ent_candidate, ent_labels=ent_labels, k_1=k_1,
                    v_1=v_1, k_2=k_2, v_2=v_2, k_cand_1=k_cand_1, v_cand_1=
                    v_cand_1, k_cand_2=k_cand_2, v_cand_2=v_cand_2,
                    cand_pos_tensor=cand_pos_tensor)
                if args.gradient_accumulation_steps > 1:
