    ...
    for x, mark in file_mark:
        ...
        for step, batch in enumerate(tqdm(eval_dataloader, desc='Iteration')):
            batch = tuple(t.to(device) if i != 3 else t for i, t in
                enumerate(batch))
            (input_ids, input_mask, segment_ids, input_ent, ent_mask, labels
                ) = batch
            input_ent += 1
            k_1, v_1, k_2, v_2 = load_k_v_queryR_small(input_ent)
            with torch.no_grad():
                if args.fp16:
                    tmp_eval_loss, _ = model(input_ids, segment_ids,
                        input_mask, input_ent, ent_mask, labels.half(), k_1
                    ...
                    tmp_eval_loss, _ = model(input_ids, segment_ids,
                        input_mask, input_ent, ent_mask, labels, k_1, v_1,
                        k_2, v_2)
                    logits = model(input_ids, segment_ids, input_mask,
                        input_ent, ent_mask, None, k_1, v_1, k_2, v_2)
        ...
        result = {'eval_loss': eval_loss, 'eval_accuracy': eval_accuracy,
            'macro': loose_macro(true, pred), 'micro': loose_micro(true, pred)}
        if mark:
