        ...
        if model_key in ckpt_keys:
            ...
            if model_weight.shape == ckpt_weight.shape:
                ...
                unmatched_log.append(
                    '*UNMATCHED* {}, Model Shape: {} <-> Ckpt Shape: {}'.
                    format(model_key, model_weight.shape, ckpt_weight.shape))
        else:
            unloaded_log.append('*UNLOADED* {}, Model Shape: {}'.format(
                model_key, model_weight.shape))
    if is_main_process():
        ...
        for key in ckpt_keys:
            logger.warning('$UNUSED$ {}, Ckpt Shape: {}'.format(key,
