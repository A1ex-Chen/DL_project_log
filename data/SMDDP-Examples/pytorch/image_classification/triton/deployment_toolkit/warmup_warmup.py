def warmup(model_name: str, batch_sizes: List[int], triton_gpu_engine_count:
    int=1, triton_instances: int=1, profiling_data: str='random',
    input_shapes: Optional[List[str]]=None, server_url: str='localhost',
    measurement_window: int=10000, shared_memory: bool=False):
    print('\n')
    ...
    exec_args = (
        f'-m {model_name}         -x 1         -p {measurement_window}         -v         -i http         -u {server_url}:8000         -b {batch_size}         --concurrency-range {min_concurrency}:{max_concurrency}:{step}         --input-data {profiling_data} {input_shapes}'
        )
    if shared_memory:
