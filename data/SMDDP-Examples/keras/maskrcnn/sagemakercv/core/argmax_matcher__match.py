def _match(self, similarity_matrix):
    ...
    def _match_when_rows_are_non_empty():
      ...
      Returns:
        matches:  int32 tensor indicating the row each column matches to.
      """
        matches = tf.argmax(input=similarity_matrix, axis=0, output_type=tf
            .int32)
        if self._matched_threshold is not None:
            matched_vals = tf.reduce_max(input_tensor=similarity_matrix, axis=0
                )
            below_unmatched_threshold = tf.greater(self.
                _unmatched_threshold, matched_vals)
            between_thresholds = tf.logical_and(tf.greater_equal(
                matched_vals, self._unmatched_threshold), tf.greater(self.
                _matched_threshold, matched_vals))
        ...
        if self._force_match_for_each_row:
            ...
            force_match_row_ids = tf.argmax(input=
                force_match_column_indicators, axis=0, output_type=tf.int32)
    ...
    if similarity_matrix.shape.is_fully_defined():
        ...
        return tf.cond(pred=tf.greater(tf.shape(input=similarity_matrix)[0],
            0), true_fn=_match_when_rows_are_non_empty, false_fn=
