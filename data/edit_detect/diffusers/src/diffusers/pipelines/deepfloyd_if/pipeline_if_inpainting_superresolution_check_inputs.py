def check_inputs(self, prompt, image, original_image, mask_image,
    batch_size, callback_steps, negative_prompt=None, prompt_embeds=None,
    ...
    if not isinstance(check_image_type, torch.Tensor) and not isinstance(
        ...
        np.ndarray):
        raise ValueError(
            f'`original_image` has to be of type `torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, or List[...] but is {type(check_image_type)}'
            )
    if isinstance(original_image, list):
    ...
    if not isinstance(check_image_type, torch.Tensor) and not isinstance(
        ...
        np.ndarray):
        raise ValueError(
            f'`mask_image` has to be of type `torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, or List[...] but is {type(check_image_type)}'
            )
    if isinstance(mask_image, list):
