def prior_original_checkpoint_to_diffusers_checkpoint(model, checkpoint):
    ...
    diffusers_checkpoint.update({'time_embedding.linear_1.weight':
        checkpoint[f'{PRIOR_ORIGINAL_PREFIX}.time_embed.c_fc.weight'],
        'time_embedding.linear_1.bias': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.time_embed.c_fc.bias']})
    diffusers_checkpoint.update({'time_embedding.linear_2.weight':
        checkpoint[f'{PRIOR_ORIGINAL_PREFIX}.time_embed.c_proj.weight'],
        'time_embedding.linear_2.bias': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.time_embed.c_proj.bias']})
    diffusers_checkpoint.update({'proj_in.weight': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.input_proj.weight'], 'proj_in.bias':
        checkpoint[f'{PRIOR_ORIGINAL_PREFIX}.input_proj.bias']})
    diffusers_checkpoint.update({'embedding_proj.weight': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.clip_embed.weight'],
        'embedding_proj.bias': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.clip_embed.bias']})
    diffusers_checkpoint.update({'positional_embedding': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.pos_emb'][None, :]})
    diffusers_checkpoint.update({'norm_in.weight': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.ln_pre.weight'], 'norm_in.bias':
        checkpoint[f'{PRIOR_ORIGINAL_PREFIX}.ln_pre.bias']})
    for idx in range(len(model.transformer_blocks)):
        diffusers_transformer_prefix = f'transformer_blocks.{idx}'
        original_transformer_prefix = (
            f'{PRIOR_ORIGINAL_PREFIX}.backbone.resblocks.{idx}')
    ...
    diffusers_checkpoint.update({'norm_out.weight': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.ln_post.weight'], 'norm_out.bias':
        checkpoint[f'{PRIOR_ORIGINAL_PREFIX}.ln_post.bias']})
    diffusers_checkpoint.update({'proj_to_clip_embeddings.weight':
        checkpoint[f'{PRIOR_ORIGINAL_PREFIX}.output_proj.weight'],
        'proj_to_clip_embeddings.bias': checkpoint[
        f'{PRIOR_ORIGINAL_PREFIX}.output_proj.bias']})
