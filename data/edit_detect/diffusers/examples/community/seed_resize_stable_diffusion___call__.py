...
def __call__(self, prompt: Union[str, List[str]], height: int=512, width:
    ...
    if do_classifier_free_guidance:
        ...
        uncond_embeddings = uncond_embeddings.repeat(batch_size,
    ...
    latents_shape_reference = (batch_size * num_images_per_prompt, self.
        unet.config.in_channels, 64, 64)
    latents_dtype = text_embeddings.dtype
    if latents is None:
        if self.device.type == 'mps':
            latents_reference = torch.randn(latents_shape_reference,
                generator=generator, device='cpu', dtype=latents_dtype).to(self
                .device)
            latents = torch.randn(latents_shape, generator=generator,
                device='cpu', dtype=latents_dtype).to(self.device)
        else:
            latents_reference = torch.randn(latents_shape_reference,
                generator=generator, device=self.device, dtype=latents_dtype)
            latents = torch.randn(latents_shape, generator=generator,
    ...
    w = latents_shape_reference[3] if dx >= 0 else latents_shape_reference[3
        ] + 2 * dx
    h = latents_shape_reference[2] if dy >= 0 else latents_shape_reference[2
        ] + 2 * dy
    tx = 0 if dx < 0 else dx
    ty = 0 if dy < 0 else dy
    dx = max(-dx, 0)
    dy = max(-dy, 0)
    latents[:, :, ty:ty + h, tx:tx + w] = latents_reference[:, :, dy:dy + h,
        dx:dx + w]
