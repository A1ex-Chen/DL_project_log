...
def __call__(self, prompt: Union[str, List[str]], negative_prompt: Optional
    [Union[str, List[str]]]=None, image: Union[torch.Tensor, PIL.Image.
    Image]=None, mask_image: Union[torch.Tensor, PIL.Image.Image]=None,
    height: int=512, width: int=512, num_inference_steps: int=50,
    guidance_scale: float=7.5, strength: float=0.8, num_images_per_prompt:
    Optional[int]=1, add_predicted_noise: Optional[bool]=False, eta: float=
    ...
    prompt_embeds = self._encode_prompt(prompt, device,
        ...
        max_embeddings_multiples, prompt_embeds=prompt_embeds,
    ...
    if isinstance(mask_image, PIL.Image.Image):
        mask_image = preprocess_mask(mask_image, batch_size, self.
            vae_scale_factor)
    ...
    timesteps, num_inference_steps = self.get_timesteps(num_inference_steps,
        strength, device, image is None)
    latent_timestep = timesteps[:1].repeat(batch_size * num_images_per_prompt)
    latents, init_latents_orig, noise = self.prepare_latents(image,
        latent_timestep, num_images_per_prompt, batch_size, self.unet.
        config.in_channels, height, width, dtype, device, generator, latents)
        ...
        for i, t in enumerate(timesteps):
            ...
            if i == len(timesteps) - 1 or i + 1 > num_warmup_steps and (i + 1
                ...
                if i % callback_steps == 0:
                    ...
                    if (is_cancelled_callback is not None and
