def parse_args(input_args=None):
    ...
    parser.add_argument('--dataset_name', type=str, default=None, help=
        'The name of the Dataset (from the HuggingFace hub) containing the training data of instance images (could be your own, possibly private, dataset). It can also be a path pointing to a local copy of a dataset in your filesystem, or to a folder containing files that 🤗 Datasets can understand.'
        )
    ...
    parser.add_argument('--do_edm_style_training', default=False, action=
        'store_true', help=
    ...
    parser.add_argument('--output_kohya_format', action='store_true', help=
        'Flag to additionally generate final state dict in the Kohya format so that it becomes compatible with A111, Comfy, Kohya, etc.'
        )
    ...
    parser.add_argument('--adam_weight_decay_text_encoder', type=float,
        default=0.001, help='Weight decay to use for text_encoder')
    ...
    parser.add_argument('--loss_type', type=str, default='l2', choices=[
        'l2', 'huber', 'smooth_l1'], help=
        "The type of loss to use and whether it's timestep-scheduled. See Issue #7488 for more info."
        )
    parser.add_argument('--huber_schedule', type=str, default='snr',
        choices=['constant', 'exponential', 'snr'], help=
        'The schedule to use for the huber losses parameter')
    parser.add_argument('--huber_c', type=float, default=0.1, help=
        'The huber loss parameter. Only used if one of the huber loss modes (huber or smooth l1) is selected with loss_type.'
        )
