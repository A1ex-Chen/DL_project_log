    ...
    try:
        for batch_idx, batch in enumerate(dl):
            ...
            optim = Optimizer.optim_generator(name=config.optim_type, lr=
                config.lr)([noise])
            lr_sched = get_cosine_with_hard_restarts_schedule_with_warmup(
                optimizer=optim, num_warmup_steps=config.num_warmup_steps,
                num_training_steps=config.num_training_steps, num_cycles=
                config.num_cycles)
            ...
            save_file({'loss_vec_log': torch.stack(loss_vec_log),
                'final_sample': pred.detach().cpu(), 'final_noise': noise.
                detach().cpu()}, os.path.join(work_dir, f'record.safetensors'))
