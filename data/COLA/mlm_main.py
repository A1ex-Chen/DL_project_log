def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_args.tokenizer_name if
        ...
        cache_dir=model_args.cache_dir, use_fast=model_args.
        use_fast_tokenizer, revision=model_args.model_revision,
        use_auth_token=True if model_args.use_auth_token else None)
    model = AutoModelForMaskedLM.from_pretrained(model_args.
    ...
    tokenize_function = get_preprocess_function(data_args, tokenizer,
        logger, token2id_mapping)
    with training_args.main_process_first(desc='dataset map tokenization'):
        tokenized_datasets = raw_datasets.map(tokenize_function, batched=
            True, num_proc=data_args.preprocessing_num_workers,
            remove_columns=column_names, load_from_cache_file=not data_args
            .overwrite_cache, desc='Running tokenizer on dataset line_by_line')
    ...
    if training_args.do_eval:
        ...
        if data_args.max_eval_samples is not None:
            max_eval_samples = min(len(valid_dataset), data_args.
                max_eval_samples)
            valid_dataset = valid_dataset.select(range(max_eval_samples))
        test_dataset = tokenized_datasets['test']
        if data_args.max_eval_samples is not None:
            max_test_samples = min(len(test_dataset), data_args.
                max_eval_samples)
    ...
    trainer = Trainer(model=model, args=training_args, train_dataset=
        train_dataset if training_args.do_train else None, eval_dataset=
        valid_dataset if training_args.do_eval else None, tokenizer=
        tokenizer, data_collator=data_collator, compute_metrics=
        compute_metrics if training_args.do_eval and not
        is_torch_tpu_available() else None, preprocess_logits_for_metrics=
        preprocess_logits_for_metrics if training_args.do_eval and not
    ...
    if training_args.do_eval:
        ...
        for eval_name, eval_dataset in zip(['valid', 'test'], [
            valid_dataset, test_dataset]):
            metrics = trainer.evaluate(eval_dataset=eval_dataset)
            metrics = {key.replace('eval', eval_name): score for key, score in
                metrics.items()}
            max_eval_samples = (data_args.max_eval_samples if data_args.
                max_eval_samples is not None else len(eval_dataset))
            metrics[f'{eval_name}_samples'] = min(max_eval_samples, len(
                eval_dataset))
            try:
