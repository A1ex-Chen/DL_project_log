def calculate_kl(featuresdict_1, featuresdict_2, feat_layer_name, same_name
    =True):
    if not same_name:
        return {'kullback_leibler_divergence_sigmoid': float(-1),
            'kullback_leibler_divergence_softmax': float(-1)}, None, None
    EPS = 1e-06
    ...
    kl_ref = torch.nn.functional.kl_div((features_1.softmax(dim=1) + EPS).
        log(), features_2.softmax(dim=1), reduction='none') / len(features_1)
    kl_ref = torch.mean(kl_ref, dim=-1)
    kl_softmax = torch.nn.functional.kl_div((features_1.softmax(dim=1) +
        EPS).log(), features_2.softmax(dim=1), reduction='sum') / len(
        features_1)
    kl_sigmoid = torch.nn.functional.kl_div((features_1.sigmoid() + EPS).
        log(), features_2.sigmoid(), reduction='sum') / len(features_1)
    return {'kullback_leibler_divergence_sigmoid': float(kl_sigmoid),
