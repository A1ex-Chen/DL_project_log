def calculate_fid(featuresdict_1, featuresdict_2, feat_layer_name):
    ...
    stat_1 = {'mu': np.mean(features_1.numpy(), axis=0), 'sigma': np.cov(
        features_1.numpy(), rowvar=False)}
    stat_2 = {'mu': np.mean(features_2.numpy(), axis=0), 'sigma': np.cov(
        features_2.numpy(), rowvar=False)}
    ...
    if not np.isfinite(covmean).all():
        print(
            f'WARNING: fid calculation produces singular product; adding {eps} to the covariance diagonal'
            )
