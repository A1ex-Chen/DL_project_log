def get_extended_attention_mask(self, attention_mask: Tensor, input_shape:
    Tuple[int], device: device, is_decoder: bool, has_query: bool=False
    ) ->Tensor:
    """
    ...
    if attention_mask.dim() == 3:
        ...
        if is_decoder:
            ...
            if causal_mask.shape[1] < attention_mask.shape[1]:
                ...
                if has_query:
                    causal_mask = torch.cat([torch.zeros((batch_size,
                        prefix_seq_len, seq_length), device=device, dtype=
                        causal_mask.dtype), causal_mask], axis=1)
                causal_mask = torch.cat([torch.ones((batch_size,
                    causal_mask.shape[1], prefix_seq_len), device=device,
