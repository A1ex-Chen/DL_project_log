def create_image_unet_diffusers_config(unet_params):
    ...
    for i in range(len(block_out_channels)):
        block_type = 'CrossAttnDownBlock2D' if unet_params.with_attn[i
            ] else 'DownBlock2D'
    ...
    up_block_types = []
    for i in range(len(block_out_channels)):
        block_type = 'CrossAttnUpBlock2D' if unet_params.with_attn[-i - 1
            ] else 'UpBlock2D'
    ...
    config = {'sample_size': None, 'in_channels': unet_params.
        input_channels, 'out_channels': unet_params.output_channels,
        'down_block_types': tuple(down_block_types), 'up_block_types':
        tuple(up_block_types), 'block_out_channels': tuple(
        block_out_channels), 'layers_per_block': unet_params.
