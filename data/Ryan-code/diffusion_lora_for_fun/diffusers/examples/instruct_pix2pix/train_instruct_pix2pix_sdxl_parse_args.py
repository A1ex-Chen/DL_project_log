def parse_args():
    ...
    parser.add_argument('--vae_precision', type=str, choices=['fp32',
        'fp16', 'bf16'], default='fp32', help=
        'The vanilla SDXL 1.0 VAE can cause NaNs due to large activation values. Some custom models might already have a solution to this problem, and this flag allows you to use mixed precision to stabilize training.'
        )
    ...
    parser.add_argument('--original_image_column', type=str, default=
        'input_image', help=
        'The column of the dataset containing the original image on which edits where made.'
        )
    parser.add_argument('--edited_image_column', type=str, default=
        'edited_image', help=
        'The column of the dataset containing the edited image.')
    parser.add_argument('--edit_prompt_column', type=str, default=
        'edit_prompt', help=
        'The column of the dataset containing the edit instruction.')
    parser.add_argument('--val_image_url_or_path', type=str, default=None,
        help=
        'URL to the original image that you would like to edit (used during inference for debugging purposes).'
        )
    ...
    parser.add_argument('--validation_steps', type=int, default=100, help=
        'Run fine-tuning validation every X steps. The validation process consists of running the prompt `args.validation_prompt` multiple times: `args.num_validation_images`.'
        )
    ...
    parser.add_argument('--conditioning_dropout_prob', type=float, default=
        None, help=
