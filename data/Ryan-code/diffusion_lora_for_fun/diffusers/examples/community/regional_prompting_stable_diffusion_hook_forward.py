def hook_forward(module):
    ...
    def forward(hidden_states: torch.Tensor, encoder_hidden_states:
        ...
        =None, temb: Optional[torch.Tensor]=None, scale: float=1.0
        ) ->torch.Tensor:
        attn = module
        ...
        if equal:
            hidden_states = torch.cat([px for i in range(regions)] + [nx for
                i in range(regions)], 0)
        ...
        if input_ndim == 4:
            ...
            hidden_states = hidden_states.view(batch_size, channel, height *
                width).transpose(1, 2)
        ...
        hidden_states = scaled_dot_product_attention(self, query, key,
                    ...
                    for icell in icells[i]:
                        if 'ROW' in mode:
                            out[0:batch, int(h * ocell[0]):int(h * ocell[1]
                                ...
                                ] = out[c * batch:(c + 1) * batch, int(h *
                                ocell[0]):int(h * ocell[1]), int(w * icell[
                                0]):int(w * icell[1]), :]
                        else:
                            out[0:batch, int(h * icell[0]):int(h * icell[1]
                                ), int(w * ocell[0]):int(w * ocell[1]), :
                                ] = out[c * batch:(c + 1) * batch, int(h *
                                icell[0]):int(h * icell[1]), int(w * ocell[
                                0]):int(w * ocell[1]), :]
                        c += 1
            px, nx = (px[0:batch], nx[0:batch]) if equal else (px[0:batch], nx)
            hidden_states = torch.cat([nx, px], 0) if revers else torch.cat([
                px, nx], 0)
            hidden_states = hidden_states.reshape(xshape)
        elif 'PRO' in mode:
            px, nx = torch.chunk(hidden_states) if equal else hidden_states[
                0:-batch], hidden_states[-batch:]
