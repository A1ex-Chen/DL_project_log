def run(self):
    directories = [directory for directory in os.listdir() if 
        'cookiecutter-template-' == directory[:22]]
    if len(directories) > 0:
        raise ValueError(
            'Several directories starting with `cookiecutter-template-` in current working directory. Please clean your directory by removing all folders startign with `cookiecutter-template-` or change your working directory.'
            )
    path_to_transformer_root = Path(__file__
        ).parent.parent.parent.parent if self._path is None else Path(self.
        _path).parent.parent
    ...
    if not self._testing:
        ...
        cookiecutter(str(path_to_cookiecutter if self._path is None else
            self._path), no_input=True, extra_context=testing_configuration)
    directory = [directory for directory in os.listdir() if 
        'cookiecutter-template-' in directory[:22]][0]
    ...
    shutil.move(f'{directory}/configuration_{lowercase_model_name}.py',
    ...
    if output_pytorch:
        ...
        shutil.move(f'{directory}/modeling_{lowercase_model_name}.py',
            f'{model_dir}/modeling_{lowercase_model_name}.py')
        shutil.move(f'{directory}/test_modeling_{lowercase_model_name}.py',
    ...
    if output_tensorflow:
        ...
        shutil.move(f'{directory}/modeling_tf_{lowercase_model_name}.py',
            f'{model_dir}/modeling_tf_{lowercase_model_name}.py')
        shutil.move(f'{directory}/test_modeling_tf_{lowercase_model_name}.py',
            f'{path_to_transformer_root}/tests/test_modeling_tf_{lowercase_model_name}.py'
            )
    else:
        os.remove(f'{directory}/modeling_tf_{lowercase_model_name}.py')
        os.remove(f'{directory}/test_modeling_tf_{lowercase_model_name}.py')
    shutil.move(f'{directory}/{lowercase_model_name}.rst',
        f'{path_to_transformer_root}/docs/source/model_doc/{lowercase_model_name}.rst'
        )
    shutil.move(f'{directory}/tokenization_{lowercase_model_name}.py',
