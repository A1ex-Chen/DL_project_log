...
import torchplus
import tDBN.data.kitti_common as kitti
from tDBN.builder import target_assigner_builder, voxel_builder
from tDBN.data.preprocess import merge_tDBN_batch_training_pass
from tDBN.data.preprocess import merge_tDBN_batch
from tDBN.protos import pipeline_pb2
from tDBN.pytorch.builder import (box_coder_builder, input_reader_builder,
...
from tDBN.core.multi_gpu_parallel import multi_gpu_parallel
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        pin_memory=False,
        collate_fn = merge_tDBN_batch_training_pass,
        ...
        for _ in range(total_loop):
            ...
            for step in range(steps):
                ...
                try:
                    ...
                    data_iter = iter(dataloader)
                    example = next(data_iter)
                # example_torch = example_convert_to_torch_training(example, float_dtype,device=device_ids[0])

                # batch_size = (example[0]["anchors"].shape[0])*2
                batch_size = len(example)
                # ret_dict = net(example_torch)
                # ret_dict  = data_parallel_tDBN(net, example_torch, device_ids=[0,1])
                ...
                if train_cfg.enable_mixed_precision:
                    loss *= loss_scale
                loss.backward()
                torch.nn.utils.clip_grad_norm_(net_paral.module.parameters(), 10.0)
                #mixed_optimizer.step()
                optimizer.module.step()  # XXX multi gpus
                optimizer.module.zero_grad()
                ...
                labels, dtype=torch.int32, device=0)
                ...
                t = time.time()
                metrics = {}
                # if 'anchors_mask' not in example_torch:   # XXX remove the
                # num_anchors
                ...
                if global_step % display_step == 0:
                    ...
                    metrics["lr"] = float(
                        optimizer.module.param_groups[0]['lr'])   # XXX remove the module,  param_groups[0]['lr'])
                ...
                if ckpt_elasped_time > train_cfg.save_checkpoints_secs:
                    torchplus.train.save_models(model_dir, [net_paral.module, optimizer.module],
                                                net_paral.module.get_global_step())
            ...
            for example in iter(eval_dataloader):
                ...
                if pickle_result:
                    dt_annos += predict_kitti_to_anno(
                        net_paral.module, example, class_names, center_limit_range,
                        model_cfg.lidar_input)
                else:
                    _predict_kitti_to_file(net_paral.module, example, result_path_step,
            ...
            print(f"avg forward time per example: {net_paral.module.avg_forward_time:.3f}")
            print(
...
def evaluate(config_path,
    ...
    for example in iter(eval_dataloader):
        example = example_convert_to_torch_tDBN(example, float_dtype)
        if pickle_result:
            dt_annos += predict_kitti_to_anno(
