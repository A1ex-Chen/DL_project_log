def _suppression_loop_body(boxes, iou_threshold, output_size, idx):
    ...
    box_slice = tf.slice(boxes, [0, idx * NMS_TILE_SIZE, 0], [batch_size,
        NMS_TILE_SIZE, 4])
    _, box_slice, _, _ = tf.while_loop(lambda _boxes, _box_slice,
        _threshold, inner_idx: inner_idx < idx, _cross_suppression, [boxes,
        box_slice, iou_threshold, tf.constant(0)])
    ...
    suppressed_iou, _, _ = tf.while_loop(lambda _iou, loop_condition,
        _iou_sum: loop_condition, _self_suppression, [iou, tf.constant(True
    ...
    output_size += tf.reduce_sum(tf.cast(tf.reduce_any(box_slice > 0, [2]),
        tf.int32), [1])
