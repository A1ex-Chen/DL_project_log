def get_anchor_center_min_dis(box_centers: torch.Tensor, anchor_centers:
    torch.Tensor):
    """
    Args:
        box_centers: [N, 2]
    ...
    if N == 0:
        return torch.ones_like(anchor_centers)[:, 0] * 99999, torch.zeros_like(
            anchor_centers)[:, 0].long()
