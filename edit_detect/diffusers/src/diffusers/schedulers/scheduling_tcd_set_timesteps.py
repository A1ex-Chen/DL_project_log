def set_timesteps(self, num_inference_steps: Optional[int]=None, device:
    ...
    None, timesteps: Optional[List[int]]=None, strength: float=1.0):
    """
    ...
    if original_inference_steps is None:
        ...
        tcd_origin_timesteps = np.asarray(list(range(1, int(original_steps *
            strength) + 1))) * k - 1
    else:
        tcd_origin_timesteps = np.asarray(list(range(0, int(self.config.
            num_train_timesteps * strength))))
    if timesteps is not None:
        ...
        elif len(timesteps) > self.config.num_train_timesteps:
            logger.warning(
                f'The number of timesteps in the custom timestep schedule is {len(timesteps)}, which exceeds the the length of the timestep schedule used for training: {self.config.num_train_timesteps}. You may get some unexpected results when using this timestep schedule.'
                )
        ...
        elif num_inference_steps > self.config.num_train_timesteps:
            raise ValueError(
                f'`num_inference_steps`: {num_inference_steps} cannot be larger than `num_train_timesteps`: {self.config.num_train_timesteps} because the final timestep schedule will be a subset of the `num_train_timesteps`-sized initial timestep schedule.'
                )
        tcd_origin_timesteps = tcd_origin_timesteps[::-1].copy()
        inference_indices = np.linspace(0, len(tcd_origin_timesteps), num=
