def _encode_prompt(self, prompt, device, num_images_per_prompt,
    ...
    text_inputs = self.tokenizer(prompt, padding='max_length', truncation=
        True, max_length=77, return_attention_mask=True, add_special_tokens
    ...
    prompt_embeds, text_encoder_hidden_states = self.text_encoder(input_ids
        =text_input_ids, attention_mask=text_mask)
    ...
    if do_classifier_free_guidance:
        ...
        uncond_input = self.tokenizer(uncond_tokens, padding='max_length',
            max_length=77, truncation=True, return_attention_mask=True,
            add_special_tokens=True, return_tensors='pt')
        uncond_text_input_ids = uncond_input.input_ids.to(device)
        uncond_text_mask = uncond_input.attention_mask.to(device)
        negative_prompt_embeds, uncond_text_encoder_hidden_states = (self.
            text_encoder(input_ids=uncond_text_input_ids, attention_mask=
            uncond_text_mask))
