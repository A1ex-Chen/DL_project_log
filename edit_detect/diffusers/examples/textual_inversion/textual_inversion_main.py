def main():
    ...
    train_dataset = TextualInversionDataset(data_root=args.train_data_dir,
        tokenizer=tokenizer, size=args.resolution, placeholder_token=' '.
        join(tokenizer.convert_ids_to_tokens(placeholder_token_ids)),
        repeats=args.repeats, learnable_property=args.learnable_property,
        center_crop=args.center_crop, set='train')
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_steps == 0:
                    weight_name = (
                        f'learned_embeds-steps-{global_step}.bin' if args.
                        no_safe_serialization else
                        f'learned_embeds-steps-{global_step}.safetensors')
                    save_path = os.path.join(args.output_dir, weight_name)
                    save_progress(text_encoder, placeholder_token_ids,
                        accelerator, args, save_path, safe_serialization=
                        not args.no_safe_serialization)
                if accelerator.is_main_process:
    ...
    if accelerator.is_main_process:
        ...
        weight_name = ('learned_embeds.bin' if args.no_safe_serialization else
            'learned_embeds.safetensors')
        save_path = os.path.join(args.output_dir, weight_name)
        save_progress(text_encoder, placeholder_token_ids, accelerator,
            args, save_path, safe_serialization=not args.no_safe_serialization)
        if args.push_to_hub:
