def parse_args():
    ...
    parser.add_argument('--progressive_tokens_max_steps', type=int, default
        =2000, help='The number of steps until all tokens will be used.')
    parser.add_argument('--progressive_tokens', action='store_true', help=
        'Progressively train the tokens. For example, first train for 1 token, then 2 tokens and so on.'
        )
    parser.add_argument('--vector_shuffle', action='store_true', help=
        'Shuffling tokens durint training')
    parser.add_argument('--num_vec_per_token', type=int, default=1, help=
        'The number of vectors used to represent the placeholder token. The higher the number, the better the result at the cost of editability. This can be fixed by prompt editing.'
        )
    parser.add_argument('--save_steps', type=int, default=500, help=
        'Save learned_embeds.bin every X updates steps.')
    parser.add_argument('--only_save_embeds', action='store_true', default=
        False, help='Save only the embeddings for the new concept.')
    ...
    parser.add_argument('--validation_epochs', type=int, default=50, help=
        'Run validation every X epochs. Validation consists of running the prompt `args.validation_prompt` multiple times: `args.num_validation_images` and logging the images.'
        )
