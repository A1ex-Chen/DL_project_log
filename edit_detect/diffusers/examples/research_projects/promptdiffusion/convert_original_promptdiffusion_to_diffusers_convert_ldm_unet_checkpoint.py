def convert_ldm_unet_checkpoint(checkpoint, config, path=None, extract_ema=
    False, controlnet=False, skip_extract_state_dict=False, promptdiffusion
    =False):
    """
    ...
    if promptdiffusion:
        ...
        new_checkpoint['controlnet_query_cond_embedding.conv_in.weight'
            ] = unet_state_dict.pop(f'input_cond_block.{orig_index}.weight')
        new_checkpoint['controlnet_query_cond_embedding.conv_in.bias'
            ] = unet_state_dict.pop(f'input_cond_block.{orig_index}.bias')
        orig_index += 2
        diffusers_index = 0
        while diffusers_index < 6:
            new_checkpoint[
            ...
            new_checkpoint[
                f'controlnet_query_cond_embedding.blocks.{diffusers_index}.weight'
                ] = unet_state_dict.pop(f'input_cond_block.{orig_index}.weight'
                )
            new_checkpoint[
                f'controlnet_query_cond_embedding.blocks.{diffusers_index}.bias'
                ] = unet_state_dict.pop(f'input_cond_block.{orig_index}.bias')
            diffusers_index += 1
            orig_index += 2
        new_checkpoint['controlnet_cond_embedding.conv_out.weight'
        ...
        new_checkpoint['controlnet_query_cond_embedding.conv_out.weight'
            ] = unet_state_dict.pop(f'input_cond_block.{orig_index}.weight')
        new_checkpoint['controlnet_query_cond_embedding.conv_out.bias'
            ] = unet_state_dict.pop(f'input_cond_block.{orig_index}.bias')
        for i in range(num_input_blocks):
            new_checkpoint[f'controlnet_down_blocks.{i}.weight'
                ] = unet_state_dict.pop(f'zero_convs.{i}.0.weight')
