def parse_args(input_args=None):
    ...
    parser.add_argument('--validation_epochs', type=int, default=1, help=
        'Run fine-tuning validation every X epochs. The validation process consists of running the prompt `args.validation_prompt` multiple times: `args.num_validation_images`.'
        )
    ...
    parser.add_argument('--timestep_bias_strategy', type=str, default=
        'none', choices=['earlier', 'later', 'range', 'none'], help=
        "The timestep bias strategy, which may help direct the model toward learning low or high frequency details. Choices: ['earlier', 'later', 'range', 'none']. The default is 'none', which means no bias is applied, and training proceeds normally. The value of 'later' will increase the frequency of the model's final training timesteps."
        )
    parser.add_argument('--timestep_bias_multiplier', type=float, default=
        1.0, help=
    ...
    parser.add_argument('--timestep_bias_begin', type=int, default=0, help=
        'When using `--timestep_bias_strategy=range`, the beginning (inclusive) timestep to bias. Defaults to zero, which equates to having no specific bias.'
        )
    parser.add_argument('--timestep_bias_end', type=int, default=1000, help
        =
        'When using `--timestep_bias_strategy=range`, the final timestep (inclusive) to bias. Defaults to 1000, which is the number of timesteps that Stable Diffusion is trained on.'
        )
    parser.add_argument('--timestep_bias_portion', type=float, default=0.25,
        help=
        'The portion of timesteps to bias. Defaults to 0.25, which 25% of timesteps will be biased. A value of 0.5 will bias one half of the timesteps. The value provided for `--timestep_bias_strategy` determines whether the biased portions are in the earlier or later timesteps.'
        )
