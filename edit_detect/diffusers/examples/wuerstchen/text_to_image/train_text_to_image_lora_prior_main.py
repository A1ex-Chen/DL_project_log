def main():
    ...
    prior_lora_config = LoraConfig(r=args.rank, lora_alpha=args.rank,
        target_modules=['to_k', 'to_q', 'to_v', 'to_out.0', 'add_k_proj',
        'add_v_proj'])
    ...
    def load_model_hook(models, input_dir):
        ...
        lora_state_dict, network_alphas = (WuerstchenPriorPipeline.
    ...
    params_to_optimize = list(filter(lambda p: p.requires_grad, prior.
        parameters()))
    ...
    if accelerator.is_main_process:
        ...
        WuerstchenPriorPipeline.save_lora_weights(save_directory=args.
            output_dir, unet_lora_layers=prior_lora_state_dict)
        images = []
        if args.validation_prompts is not None:
            logger.info('Running inference for collecting generated images...')
            pipeline = AutoPipelineForText2Image.from_pretrained(args.
                pretrained_decoder_model_name_or_path, prior_text_encoder=
                accelerator.unwrap_model(text_encoder), prior_tokenizer=
                tokenizer, torch_dtype=weight_dtype)
            pipeline = pipeline.to(accelerator.device)
            pipeline.prior_pipe.load_lora_weights(args.output_dir,
