def parse_args():
    ...
    parser.add_argument('--pretrained_teacher_model', type=str, default=
        None, required=True, help=
        'Path to pretrained LDM teacher model or model identifier from huggingface.co/models.'
        )
    ...
    parser.add_argument('--teacher_revision', type=str, default=None,
        required=False, help=
        'Revision of pretrained LDM teacher model identifier from huggingface.co/models.'
        )
    parser.add_argument('--revision', type=str, default=None, required=
        False, help=
        'Revision of pretrained LDM model identifier from huggingface.co/models.'
        )
    parser.add_argument('--output_dir', type=str, default=
        'lcm-xl-distilled', help=
    ...
    parser.add_argument('--train_shards_path_or_url', type=str, default=
        None, help=
    ...
    parser.add_argument('--use_fix_crop_and_size', action='store_true',
        help=
        'Whether or not to use the fixed crop and size for the teacher model.',
        default=False)
    ...
    parser.add_argument('--w_min', type=float, default=3.0, required=False,
        help=
        'The minimum guidance scale value for guidance scale sampling. Note that we are using the Imagen CFG formulation rather than the LCM formulation, which means all guidance scales have 1 added to them as compared to the original paper.'
        )
    parser.add_argument('--w_max', type=float, default=15.0, required=False,
        help=
        'The maximum guidance scale value for guidance scale sampling. Note that we are using the Imagen CFG formulation rather than the LCM formulation, which means all guidance scales have 1 added to them as compared to the original paper.'
        )
    parser.add_argument('--num_ddim_timesteps', type=int, default=50, help=
        'The number of timesteps to use for DDIM sampling.')
    parser.add_argument('--loss_type', type=str, default='l2', choices=[
        'l2', 'huber'], help='The type of loss to use for the LCD loss.')
    parser.add_argument('--huber_c', type=float, default=0.001, help=
        'The huber loss parameter. Only used if `--loss_type=huber`.')
    parser.add_argument('--lora_rank', type=int, default=64, help=
        'The rank of the LoRA projection matrix.')
    parser.add_argument('--lora_alpha', type=int, default=64, help=
        'The value of the LoRA alpha parameter, which controls the scaling factor in front of the LoRA weight update delta_W. No scaling will be performed if this value is equal to `lora_rank`.'
        )
    parser.add_argument('--lora_dropout', type=float, default=0.0, help=
        'The dropout probability for the dropout layer added before applying the LoRA to each layer input.'
        )
    parser.add_argument('--lora_target_modules', type=str, default=None,
        help=
        'A comma-separated string of target module keys to add LoRA to. If not set, a default list of modules will be used. By default, LoRA will be applied to all conv and linear layers.'
        )
    parser.add_argument('--vae_encode_batch_size', type=int, default=8,
        required=False, help=
        'The batch size used when encoding (and decoding) images to latents (and vice versa) using the VAE. Encoding or decoding the whole batch at once may run into OOM issues.'
        )
    parser.add_argument('--timestep_scaling_factor', type=float, default=
        10.0, help=
        'The multiplicative timestep scaling factor used when calculating the boundary scalings for LCM. The higher the scaling is, the lower the approximation error, but the default value of 10.0 should typically suffice.'
        )
    ...
    parser.add_argument('--cast_teacher_unet', action='store_true', help=
        'Whether to cast the teacher U-Net to the precision specified by `--mixed_precision`.'
        )
