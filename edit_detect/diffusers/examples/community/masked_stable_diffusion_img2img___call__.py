...
def __call__(self, prompt: Union[str, List[str]]=None, image: Union[torch.
    ...
    .Image], List[np.ndarray]]=None, strength: float=0.8,
    ...
    Optional[Dict[str, Any]]=None, mask: Union[torch.Tensor, PIL.Image.
    Image, np.ndarray, List[torch.Tensor], List[PIL.Image.Image], List[np.
    ndarray]]=None):
    """
    ...
    init_latents = [self.vae.encode(image.to(device=device, dtype=
        prompt_embeds.dtype)[i:i + 1]).latent_dist.mean for i in range(
        batch_size)]
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            if latent_mask is not None:
                latents = torch.lerp(init_latents * self.vae.config.
                    scaling_factor, latents, latent_mask)
                noise_pred = torch.lerp(torch.zeros_like(noise_pred),
                    noise_pred, latent_mask)
    ...
    if not output_type == 'latent':
        ...
        if self.debug_save:
            image_gen = self.vae.decode(latents / self.vae.config.
                scaling_factor, return_dict=False)[0]
            image_gen = self.image_processor.postprocess(image_gen,
                output_type=output_type, do_denormalize=[True])
