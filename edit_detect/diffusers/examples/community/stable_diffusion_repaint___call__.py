...
def __call__(self, prompt: Union[str, List[str]]=None, image: Union[torch.
    ...
    None, num_inference_steps: int=50, jump_length: Optional[int]=10,
    jump_n_sample: Optional[int]=10, guidance_scale: float=7.5,
    ...
    if num_channels_latents != self.unet.config.in_channels:
        raise ValueError(
            f'Incorrect configuration settings! The config of `pipeline.unet`: {self.unet.config} expects {self.unet.config.in_channels} but received `num_channels_latents`: {num_channels_latents}  = Please verify the config of `pipeline.unet` or your `mask_image` or `image` input.'
            )
    ...
    with self.progress_bar(total=len(timesteps)) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            latents = self.scheduler.step(noise_pred, t, latents,
                masked_image_latents, mask, **extra_step_kwargs).prev_sample
            progress_bar.update()
