...
def __call__(self, prompt: Union[str, List[str]]=None, height: Optional[int
    ...
    timesteps = [((1.0 - i / num_inference_steps) * 1000.0) for i in range(
        num_inference_steps)]
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            vec_t = torch.ones((latent_model_input.shape[0],), device=
                latents.device) * t
            v_pred = self.unet(latent_model_input, vec_t,
                encoder_hidden_states=prompt_embeds).sample
            if do_classifier_free_guidance:
                v_pred_neg, v_pred_text = v_pred.chunk(2)
                v_pred = v_pred_neg + guidance_scale * (v_pred_text -
                    v_pred_neg)
