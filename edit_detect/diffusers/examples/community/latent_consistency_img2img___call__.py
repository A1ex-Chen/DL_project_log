...
def __call__(self, prompt: Union[str, List[str]]=None, image:
    PipelineImageInput=None, strength: float=0.8, height: Optional[int]=768,
    width: Optional[int]=768, guidance_scale: float=7.5,
    ...
    self.scheduler.set_timesteps(strength, num_inference_steps,
        lcm_origin_steps)
    ...
    if latents is None:
        latents = self.prepare_latents(image, latent_timestep, batch_size *
            ...
            prompt_embeds.dtype, device, latents)
