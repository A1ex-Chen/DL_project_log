def convert_models(onnx_path: str, num_controlnet: int, output_path: str,
    fp16: bool=False, sd_xl: bool=False):
    """
    ...
    if sd_xl:
        ...
        img_size = 512
        batch_size = 1
    latents_shape = (2 * batch_size, unet_in_channels, unet_sample_size,
        unet_sample_size)
    embed_shape = 2 * batch_size, num_tokens, text_hidden_size
    controlnet_conds_shape = (num_controlnet, 2 * batch_size, 3, img_size,
        img_size)
    TRT_LOGGER = trt.Logger(trt.Logger.VERBOSE)
    TRT_BUILDER = trt.Builder(TRT_LOGGER)
    TRT_RUNTIME = trt.Runtime(TRT_LOGGER)
    network = TRT_BUILDER.create_network(1 << int(trt.
    ...
    profile.set_shape('encoder_hidden_states', embed_shape, embed_shape,
        embed_shape)
    profile.set_shape('controlnet_conds', controlnet_conds_shape,
        controlnet_conds_shape, controlnet_conds_shape)
    if sd_xl:
        profile.set_shape('text_embeds', text_embeds_shape,
            text_embeds_shape, text_embeds_shape)
        profile.set_shape('time_ids', time_ids_shape, time_ids_shape,
            time_ids_shape)
    config = TRT_BUILDER.create_builder_config()
    config.add_optimization_profile(profile)
    config.set_preview_feature(trt.PreviewFeature.
        DISABLE_EXTERNAL_TACTIC_SOURCES_FOR_CORE_0805, True)
    if fp16:
