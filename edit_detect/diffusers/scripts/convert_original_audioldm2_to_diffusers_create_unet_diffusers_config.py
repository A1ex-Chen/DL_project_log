def create_unet_diffusers_config(original_config, image_size: int):
    ...
    cross_attention_dim = list(unet_params['context_dim']
        ) if 'context_dim' in unet_params else block_out_channels
    if len(cross_attention_dim) > 1:
        cross_attention_dim = [cross_attention_dim for _ in range(len(
            block_out_channels))]
    config = {'sample_size': image_size // vae_scale_factor, 'in_channels':
        ...
        'num_res_blocks'], 'transformer_layers_per_block': unet_params[
        'transformer_depth'], 'cross_attention_dim': tuple(cross_attention_dim)
        }
    return config
