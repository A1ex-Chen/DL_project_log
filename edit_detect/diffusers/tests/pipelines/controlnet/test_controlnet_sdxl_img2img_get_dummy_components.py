def get_dummy_components(self, skip_first_text_encoder=False):
    ...
    torch.manual_seed(0)
    controlnet = ControlNetModel(block_out_channels=(32, 64),
        layers_per_block=2, in_channels=4, down_block_types=('DownBlock2D',
        'CrossAttnDownBlock2D'), conditioning_embedding_out_channels=(16, 
        32), attention_head_dim=(2, 4), use_linear_projection=True,
        addition_embed_type='text_time', addition_time_embed_dim=8,
        transformer_layers_per_block=(1, 2),
    ...
    components = {'unet': unet, 'controlnet': controlnet, 'scheduler':
        scheduler, 'vae': vae, 'text_encoder': text_encoder if not
        skip_first_text_encoder else None, 'tokenizer': tokenizer if not
        skip_first_text_encoder else None, 'text_encoder_2': text_encoder_2,
