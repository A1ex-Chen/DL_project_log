def test_stable_diffusion_inpaint_pipeline(self):
    ...
    prompt_ids, processed_masked_images, processed_masks = (pipeline.
        prepare_inputs(prompt, init_image, mask_image))
    params = replicate(params)
    prng_seed = jax.random.split(prng_seed, jax.device_count())
    prompt_ids = shard(prompt_ids)
    processed_masked_images = shard(processed_masked_images)
    processed_masks = shard(processed_masks)
    output = pipeline(prompt_ids, processed_masks, processed_masked_images,
        params, prng_seed, num_inference_steps, jit=True)
    ...
    expected_slice = jnp.array([0.3611307, 0.37649736, 0.3757408, 
        0.38213953, 0.39295167, 0.3841631, 0.41554978, 0.4137475, 0.4217084])
