def reconstruct_x0(pipeline: DiffusionPipeline, x0: torch.Tensor, t: torch.
    Tensor, noise: torch.Tensor=None, generator: torch.Generator=0
    ) ->torch.Tensor:
    if x0.dim() != 4:
        x0 = x0.unsqueeze(0)
    num: int = x0.shape[0]
    ...
    pred_original_sample = reconstruct_xt(pipeline=pipeline, xt=
        alpha_prod_t ** 0.5 * x0 + scaled_noise, t=t)
