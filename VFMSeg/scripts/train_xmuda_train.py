def train(cfg, args, output_dir='', run_name=''):
    ...
    val_dataloader = build_dataloader(cfg, args, mode='val', domain='target'
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        if cfg.TRAIN.XMUDA.lambda_minent > 0:
            ...
            minent_loss_trg_3d = entropy_loss(F.softmax(preds_3d[
                'seg_logit'], dim=1))
            train_metric_logger.update(minent_loss_trg_2d=
        ...
        if val_period > 0 and (cur_iter % val_period == 0 or cur_iter ==
            ...
            setup_validate()
            validate(cfg, None, model_2d, model_3d, val_dataloader,
                val_metric_logger)
