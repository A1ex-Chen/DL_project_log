def collate_scn_base(input_dict_list, output_orig, with_vfm, output_image=True
    ...
    for idx, input_dict in enumerate(input_dict_list):
        ...
        if output_pselab:
            pseudo_label_2d.append(torch.from_numpy(input_dict[
                'pseudo_label_2d']))
            if input_dict['pseudo_label_3d'] is not None:
                pseudo_label_3d.append(torch.from_numpy(input_dict[
                    'pseudo_label_3d']))
        if with_vfm:
            img_paths.append(input_dict['img_paths'])
            img_instances_orig.append(torch.from_numpy(input_dict[
                'img_instances_orig']))
            img_indices_orig.append(input_dict['img_indices_orig'])
    ...
    if output_image:
        ...
        if with_vfm:
            out_dict['img_paths'] = img_paths
            out_dict['img_indices_orig'] = img_indices_orig
            out_dict['img_instances_orig'] = img_instances_orig
    if output_orig:
