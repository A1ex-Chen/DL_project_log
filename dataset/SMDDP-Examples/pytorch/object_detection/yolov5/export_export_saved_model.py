def export_saved_model(model, im, file, dynamic, tf_nms=False, agnostic_nms
    ...
    try:
        ...
        _ = tf_model.predict(im, tf_nms, agnostic_nms, topk_per_class,
            topk_all, iou_thres, conf_thres)
        inputs = tf.keras.Input(shape=(*imgsz, ch), batch_size=None if
            dynamic else batch_size)
        ...
        if keras:
            ...
            tfm = tf.Module()
            tfm.__call__ = tf.function(lambda x: frozen_func(x)[:4] if
                tf_nms else frozen_func(x)[0], [spec])
            tfm.__call__(im)
