    ...
    if self.comet_log_per_class_metrics:
        if self.num_classes > 1:
            for i, c in enumerate(ap_class):
                ...
                self.experiment.log_metrics({'mAP@.5': ap50[i],
                    'mAP@.5:.95': ap[i], 'precision': p[i], 'recall': r[i],
                    'f1': f1[i], 'true_positives': tp[i], 'false_positives':
                    fp[i], 'support': nt[c]}, prefix=class_name)
