def forward(self, pred_dist, pred_bboxes_lrtb, pred_bboxes, t_pred_dist,
    t_pred_bboxes, temperature, anchor_points, target_bboxes, target_scores,
    ...
    if num_pos > 0:
        ...
        pred_bboxes_pos = torch.masked_select(pred_bboxes, bbox_mask).reshape([
            -1, 4])
        pred_bboxes_lrtb_pos = torch.masked_select(pred_bboxes_lrtb, bbox_mask
            ).reshape([-1, 4])
        ...
        loss_iou_lrtb = self.iou_loss(pred_bboxes_lrtb_pos, target_bboxes_pos
            ) * bbox_weight
