...
@add_code_sample_docstrings(tokenizer_class=_TOKENIZER_FOR_DOC, checkpoint=
    'distilbert-base-uncased', output_type=TFQuestionAnsweringModelOutput,
    config_class=_CONFIG_FOR_DOC)
def call(self, input_ids=None, attention_mask=None, head_mask=None,
    ...
    return_dict = inputs['return_dict'] if inputs['return_dict'
        ] is not None else self.distilbert.return_dict
    distilbert_output = self.distilbert(input_ids=inputs['input_ids'],
        attention_mask=inputs['attention_mask'], head_mask=inputs[
        'head_mask'], inputs_embeds=inputs['inputs_embeds'],
    ...
    return TFQuestionAnsweringModelOutput(loss=loss, start_logits=
        ...
        distilbert_output.hidden_states, attentions=distilbert_output.
        attentions)
