def create_groundtruth_database(data_path, info_path=None, used_classes=
    None, database_save_path=None, db_info_save_path=None, relative_path=
    True, lidar_only=False, bev_only=False, coors_range=None):
    ...
    for info in prog_bar(kitti_infos):
        ...
        points = np.fromfile(velodyne_path, dtype=np.float32, count=-1
        ...
        if not lidar_only:
            points = box_np_ops.remove_outside_points(points, rect, Trv2c,
                P2, info['img_shape'])
        annos = info['annos']
        names = annos['name']
        bboxes = annos['bbox']
        ...
        for i in range(num_obj):
            ...
            if names[i] in used_classes:
                ...
                db_info = {'name': names[i], 'path': db_path, 'image_idx':
                    image_idx, 'gt_idx': gt_idxes[i], 'box3d_lidar':
                    rbbox_lidar[i], 'num_points_in_gt': gt_points.shape[0],
