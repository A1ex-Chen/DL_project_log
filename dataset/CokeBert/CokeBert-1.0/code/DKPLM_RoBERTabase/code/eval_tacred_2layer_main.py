...
"""
    ...
    for x, mark in file_mark:
        ...
        if mark:
            eval_features = dev
            output_file = os.path.join(args.output_dir, 'eval_pred_{}.txt'.
                format(x.split('_')[-1]))
            output_file_ = os.path.join(args.output_dir, 'eval_gold_{}.txt'
                .format(x.split('_')[-1]))
        else:
            eval_features = test
            output_file = os.path.join(args.output_dir, 'test_pred_{}.txt'.
                format(x.split('_')[-1]))
            output_file_ = os.path.join(args.output_dir, 'test_gold_{}.txt'
                .format(x.split('_')[-1]))
        ...
        for input_ids, input_mask, segment_ids, input_ent, ent_mask, label_ids in eval_dataloader:
            ...
            if int(input_ent[input_ent != 0].shape[0]) == 0:
                ...
                k_2 = torch.zeros(input_ent.shape[0], 1, 1, 1, 100).cuda(
                    ).half()
                v_2 = torch.zeros(input_ent.shape[0], 1, 1, 1, 100).cuda(
                    ).half()
            """
            else:
                #print(k_2)
