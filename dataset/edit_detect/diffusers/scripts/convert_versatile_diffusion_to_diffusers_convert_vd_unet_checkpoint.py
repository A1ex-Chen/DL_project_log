def convert_vd_unet_checkpoint(checkpoint, config, unet_key, extract_ema=False
    ...
    for i in range(1, num_input_blocks):
        ...
        if f'input_blocks.{i}.0.op.weight' in unet_state_dict:
            ...
            if shape[0] != shape[1]:
                continue
            new_checkpoint[f'down_blocks.{block_id}.downsamplers.0.weight'
                ] = unet_state_dict.pop(f'input_blocks.{i}.0.weight')
            new_checkpoint[f'down_blocks.{block_id}.downsamplers.0.bias'
    ...
    for i in range(num_output_blocks):
        ...
        if len(output_block_list) > 1:
            ...
            if ['conv.weight', 'conv.bias'] in output_block_list.values():
                ...
                if shape[0] != shape[1]:
                    continue
                new_checkpoint[f'up_blocks.{block_id}.upsamplers.0.weight'
                    ] = unet_state_dict.pop(f'output_blocks.{i}.1.weight')
                new_checkpoint[f'up_blocks.{block_id}.upsamplers.0.bias'
                    ] = unet_state_dict.pop(f'output_blocks.{i}.1.bias')
                if len(attentions) == 2:
                    attentions = []
                ...
                if shape[0] != shape[1]:
                    continue
                new_checkpoint[f'up_blocks.{block_id}.upsamplers.0.weight'
                    ] = unet_state_dict.pop(f'output_blocks.{i}.2.weight')
                new_checkpoint[f'up_blocks.{block_id}.upsamplers.0.bias'
                    ] = unet_state_dict.pop(f'output_blocks.{i}.2.bias')
            if len(attentions):
                paths = renew_attention_paths(attentions)
