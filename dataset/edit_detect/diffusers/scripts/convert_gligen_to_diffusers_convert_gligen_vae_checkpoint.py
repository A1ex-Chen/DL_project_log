def convert_gligen_vae_checkpoint(checkpoint, config):
    ...
    for key in new_checkpoint.keys():
        if ('encoder.mid_block.attentions.0' in key or 
            'decoder.mid_block.attentions.0' in key):
            if 'query' in key:
                new_checkpoint[key.replace('query', 'to_q')
                    ] = new_checkpoint.pop(key)
            if 'value' in key:
                new_checkpoint[key.replace('value', 'to_v')
                    ] = new_checkpoint.pop(key)
            if 'key' in key:
                new_checkpoint[key.replace('key', 'to_k')
                    ] = new_checkpoint.pop(key)
            if 'proj_attn' in key:
                new_checkpoint[key.replace('proj_attn', 'to_out.0')
                    ] = new_checkpoint.pop(key)
