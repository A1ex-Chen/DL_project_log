def vqvae_decoder_to_diffusers_checkpoint(model, checkpoint):
    ...
    for diffusers_up_block_idx, up_block in enumerate(model.decoder.up_blocks):
        ...
        for resnet_idx, resnet in enumerate(up_block.resnets):
            ...
            diffusers_checkpoint.update(vqvae_resnet_to_diffusers_checkpoint
        ...
        if hasattr(up_block, 'attentions'):
            for attention_idx, _ in enumerate(up_block.attentions):
                ...
                diffusers_checkpoint.update(
                    vqvae_attention_to_diffusers_checkpoint(checkpoint,
    ...
    diffusers_checkpoint.update(vqvae_attention_to_diffusers_checkpoint(
    ...
    for diffusers_resnet_idx, resnet in enumerate(model.encoder.mid_block.
        ...
        diffusers_checkpoint.update(vqvae_resnet_to_diffusers_checkpoint(
            resnet, checkpoint, diffusers_resnet_prefix=
            diffusers_resnet_prefix, resnet_prefix=resnet_prefix))
    diffusers_checkpoint.update({'decoder.conv_norm_out.weight': checkpoint
        ['decoder.norm_out.weight'], 'decoder.conv_norm_out.bias':
        checkpoint['decoder.norm_out.bias'], 'decoder.conv_out.weight':
