def step(self, model_output: torch.Tensor, timestep: Union[float, torch.
    ...
    if isinstance(timestep, int) or isinstance(timestep, torch.IntTensor
        ...
        raise ValueError(
            'Passing integer indices (e.g. from `enumerate(timesteps)`) as timesteps to `EDMEulerScheduler.step()` is not supported. Make sure to pass one of the `scheduler.timesteps` as a timestep.'
            )
    ...
    pred_original_sample = self.precondition_outputs(sample, model_output,
        sigma_hat)
    ...
    if not return_dict:
        return prev_sample,
    return EDMEulerSchedulerOutput(prev_sample=prev_sample,
