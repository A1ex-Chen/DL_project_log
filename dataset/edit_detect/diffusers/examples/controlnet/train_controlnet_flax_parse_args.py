def parse_args():
    ...
    parser.add_argument('--controlnet_model_name_or_path', type=str,
        default=None, help=
        'Path to pretrained controlnet model or model identifier from huggingface.co/models. If not specified controlnet weights are initialized from unet.'
        )
    parser.add_argument('--revision', type=str, default=None, help=
        'Revision of pretrained model identifier from huggingface.co/models.')
    parser.add_argument('--from_pt', action='store_true', help=
        'Load the pretrained model from a PyTorch checkpoint.')
    parser.add_argument('--controlnet_revision', type=str, default=None,
        help=
        'Revision of controlnet model identifier from huggingface.co/models.')
    parser.add_argument('--profile_steps', type=int, default=0, help=
        'How many training steps to profile in the beginning.')
    parser.add_argument('--profile_validation', action='store_true', help=
        'Whether to profile the (last) validation.')
    parser.add_argument('--profile_memory', action='store_true', help=
        'Whether to dump an initial (before training loop) and a final (at program end) memory profile.'
        )
    parser.add_argument('--ccache', type=str, default=None, help=
        'Enables compilation cache.')
    parser.add_argument('--controlnet_from_pt', action='store_true', help=
        'Load the controlnet model from a PyTorch checkpoint.')
    parser.add_argument('--tokenizer_name', type=str, default=None, help=
        'Pretrained tokenizer name or path if not the same as model_name')
    parser.add_argument('--output_dir', type=str, default=
        'runs/{timestamp}', help=
        'The output directory where the model predictions and checkpoints will be written. Can contain placeholders: {timestamp}.'
        )
    ...
    parser.add_argument('--scale_lr', action='store_true', help=
    ...
    parser.add_argument('--logging_steps', type=int, default=100, help=
    ...
    parser.add_argument('--streaming', action='store_true', help=
        'To stream a large dataset from Hub.')
    ...
    parser.add_argument('--train_data_dir', type=str, default=None, help=
        'A folder containing the training dataset. By default it will use `load_dataset` method to load a custom dataset from the folder.Folder must contain a dataset script as described here https://huggingface.co/docs/datasets/dataset_script) .If `--load_from_disk` flag is passed, it will use `load_from_disk` method instead. Ignored if `dataset_name` is specified.'
        )
    parser.add_argument('--load_from_disk', action='store_true', help=
        'If True, will load a dataset that was previously saved using `save_to_disk` from `--train_data_dir`See more https://huggingface.co/docs/datasets/package_reference/main_classes#datasets.Dataset.load_from_disk'
        )
    parser.add_argument('--image_column', type=str, default='image', help=
        'The column of the dataset containing the target image.')
    parser.add_argument('--conditioning_image_column', type=str, default=
        'conditioning_image', help=
    ...
    parser.add_argument('--max_train_samples', type=int, default=None, help
        =
        'For debugging purposes or quicker training, truncate the number of training examples to this value if set. Needed if `streaming` is set to True.'
        )
    ...
    parser.add_argument('--validation_prompt', type=str, default=None,
        nargs='+', help=
        'A set of prompts evaluated every `--validation_steps` and logged to `--report_to`. Provide either a matching number of `--validation_image`s, a single `--validation_image` to be used with all prompts, or a single prompt that will be used with all `--validation_image`s.'
        )
    ...
    parser.add_argument('--validation_steps', type=int, default=100, help=
        'Run validation every X steps. Validation consists of running the prompt `args.validation_prompt` and logging the images.'
        )
    parser.add_argument('--wandb_entity', type=str, default=None, help=
        'The wandb entity to use (for teams).')
    parser.add_argument('--tracker_project_name', type=str, default=
        'train_controlnet_flax', help='The `project` argument passed to wandb')
    parser.add_argument('--gradient_accumulation_steps', type=int, default=
        1, help='Number of steps to accumulate gradients over')
    parser.add_argument('--local_rank', type=int, default=-1, help=
        'For distributed training: local_rank')
    args = parser.parse_args()
    args.output_dir = args.output_dir.replace('{timestamp}', time.strftime(
        '%Y%m%d_%H%M%S'))
    ...
    if (args.validation_image is not None and args.validation_prompt is not
        None and len(args.validation_image) != 1 and len(args.
        validation_prompt) != 1 and len(args.validation_image) != len(args.
        validation_prompt)):
        raise ValueError(
            'Must provide either 1 `--validation_image`, 1 `--validation_prompt`, or the same number of `--validation_prompt`s and `--validation_image`s'
            )
