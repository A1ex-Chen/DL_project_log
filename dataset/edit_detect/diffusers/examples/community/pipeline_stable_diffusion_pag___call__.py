...
def __call__(self, prompt: Union[str, List[str]]=None, height: Optional[int
    ...
    timesteps: List[int]=None, guidance_scale: float=7.5, pag_scale: float=
    0.0, pag_adaptive_scaling: float=0.0, pag_applied_layers_index: List[
    str]=['d4'], negative_prompt: Optional[Union[str, List[str]]]=None,
    ...
    prompt_embeds, negative_prompt_embeds = self.encode_prompt(prompt,
        ...
        lora_scale, clip_skip=self.clip_skip)
    ...
    if self.do_perturbed_attention_guidance:
        ...
        for drop_layer in drop_layers:
            try:
                if drop_layer[0] == 'd':
                    down_layers[int(drop_layer[1])
                        ].processor = replace_processor
                elif drop_layer[0] == 'm':
                    mid_layers[int(drop_layer[1])
                        ].processor = replace_processor
                elif drop_layer[0] == 'u':
        ...
        for i, t in enumerate(timesteps):
            ...
            if (self.do_classifier_free_guidance and not self.
                ...
                if self.do_pag_adaptive_scaling:
                    signal_scale = (self.pag_scale - self.
                        pag_adaptive_scaling * (1000 - t))
                    if signal_scale < 0:
                        signal_scale = 0
                ...
                if self.do_pag_adaptive_scaling:
                    ...
                    if signal_scale < 0:
                        signal_scale = 0
                noise_pred = noise_pred_text + (self.guidance_scale - 1.0) * (
                    noise_pred_text - noise_pred_uncond) + signal_scale * (
