...
def __call__(self, prompt: Union[str, List[str]], image: Union[torch.Tensor,
    PIL.Image.Image], text: str, height: int=512, width: int=512,
    ...
    Tensor], None]]=None, callback_steps: int=1, **kwargs):
    """
        ...
        """
    inputs = self.segmentation_processor(text=[text], images=[image],
    ...
    inpainting_pipeline = StableDiffusionInpaintPipeline(vae=self.vae,
        text_encoder=self.text_encoder, tokenizer=self.tokenizer, unet=self
        .unet, scheduler=self.scheduler, safety_checker=self.safety_checker,
        feature_extractor=self.feature_extractor)
    return inpainting_pipeline(prompt=prompt, image=image, mask_image=
        mask_pil, height=height, width=width, num_inference_steps=
