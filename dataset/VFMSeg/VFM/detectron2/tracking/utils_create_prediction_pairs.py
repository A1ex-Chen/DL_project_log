def create_prediction_pairs(instances: Instances, prev_instances: Instances,
    iou_all: np.ndarray, threshold: float=0.5) ->List:
    """
    Args:
    ...
    for i in range(len(instances)):
        for j in range(len(prev_instances)):
            if iou_all[i, j] < threshold:
                continue
            bbox_pairs.append({'idx': i, 'prev_idx': j, 'prev_id':
                prev_instances.ID[j], 'IoU': iou_all[i, j], 'prev_period':
                prev_instances.ID_period[j]})
    return bbox_pairs
