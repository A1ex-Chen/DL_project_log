...
- runwayml/stable-diffusion-inpainting 
 you should change 'sample_size' to 64 in the configuration file. Please make sure to update the config accordingly as leaving `sample_size=32` in the config might lead to incorrect results in future versions. If you have downloaded this checkpoint from the Hugging Face Hub, it would be very nice if you could open a Pull request for the `unet/config.json` file"""
    ...
    if unet.config.in_channels != 9:
        logger.warning(
            f'You have loaded a UNet with {unet.config.in_channels} input channels, whereas by default, {self.__class__} assumes that `pipeline.unet` has 9 input channels: 4 for `num_channels_latents`, 1 for `num_channels_mask`, and 4 for `num_channels_masked_image`. If you did not intend to modify this behavior, please check whether you have loaded the right checkpoint.'
            )
