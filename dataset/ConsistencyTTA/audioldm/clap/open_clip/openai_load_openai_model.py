def load_openai_model(name: str, model_cfg, device: Union[str, torch.device
    ]='cuda' if torch.cuda.is_available() else 'cpu', jit=True, cache_dir=
    os.path.expanduser(f'{CACHE_DIR}/clip'), enable_fusion: bool=False,
    ...
    if not jit:
        try:
            model = build_model_from_openai_state_dict(state_dict or model.
                state_dict(), model_cfg, enable_fusion, fusion_type).to(device)
        except KeyError:
            sd = {k[7:]: v for k, v in state_dict['state_dict'].items()}
            model = build_model_from_openai_state_dict(sd, model_cfg,
                enable_fusion, fusion_type).to(device)
        if str(device) == 'cpu':
            model.float()
        return model
