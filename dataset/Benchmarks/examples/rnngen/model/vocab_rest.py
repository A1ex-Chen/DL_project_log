    ...
    with open(args.i, "r") as f:
        with open(args.o + "/out.txt", "w") as o:
            with multiprocessing.Pool(args.p) as p:
                smiss = p.imap(
                    partial(
                        randomSmiles, max_len=args.maxlen, attempts=args.permute_smiles
                    ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", type=str, required=True, help="input file with single smile per line"
    )
    parser.add_argument(
        "-o",
        type=str,
        required=True,
        help="output directory where preprocressed data and vocab will go",
    )
    parser.add_argument("--start", action="store_true")
    parser.add_argument(
        "--maxlen", type=int, required=True, help="max length for smile strings to be"
    )
    parser.add_argument(
        "--permute_smiles", type=int, help="generates permutations of smiles", default=0
    )
    ...
    try:
        ...
        print(
            "Creation of the directory %s failed. Maybe it already exists? I will overwrite :)"
            % path
        )
    else:
