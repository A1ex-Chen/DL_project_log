def run(gParams):
    ...
    train_file = candle.fetch_file(data_url + train_data, subdir=
        'Examples/image_vae')
    test_file = candle.fetch_file(data_url + test_data, subdir=
        'Examples/image_vae')
    starting_epoch = 1
    ...
    if torch.cuda.device_count() > 1:
        data_para = True
    cuda = True
    device = torch.device('cuda' if cuda and torch.cuda.is_available() else
        'cpu')
    ...
    train_loader_food = torch.utils.data.DataLoader(train_data, batch_size=
        args.batch_size, shuffle=True, drop_last=True, **kwargs)
    val_data = MoleLoader(smiles_lookup_test)
    val_loader_food = torch.utils.data.DataLoader(val_data, batch_size=args
        .batch_size, shuffle=True, drop_last=True, **kwargs)
        ...
        for batch_idx, (_, data, _) in enumerate(train_loader_food):
            ...
            if batch_idx % log_interval == 0:
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} {}'.
                    ...
                    train_loader_food.dataset), 100.0 * batch_idx / len(
                    train_loader_food), loss_meter.avg, datetime.datetime.
                    now()))
    ...
    def test(epoch, args):
        ...
        with torch.no_grad():
            for i, (_, data, _) in enumerate(val_loader_food):
                data = data.float().cuda()
                recon_batch, mu, logvar, _ = model(data)
                print('recon', recon_batch.shape, mu.shape, logvar.shape,
                    data.shape)
                loss2 = loss_picture(recon_batch, data, mu, logvar, epoch)
                loss2 = torch.sum(loss2)
                ...
                if i == 0:
                    ...
                    for i in range(n_image_gen):
                        ...
                        sample_vec = interpolate_points(pt_1, pt_2, np.
                            linspace(0, 1, num=n_samples_linspace, endpoint
                            =True))
    ...
    for epoch in trn_rng:
        ...
        test(epoch, args)
        if data_para:
            torch.save({'epoch': epoch, 'encoder_state_dict': model.module.
                encoder.state_dict(), 'decoder_state_dict': model.module.
                decoder.state_dict(), 'optimizer_state_dict': optimizer.
                state_dict()}, save_files + '/epoch_' + str(epoch) + '.pt')
            torch.save(model.module, 'model_inf.pt')
        else:
            torch.save({'epoch': epoch, 'encoder_state_dict': model.encoder
                .state_dict(), 'decoder_state_dict': model.decoder.
        ...
        with torch.no_grad():
            ...
            save_image(sample.view(64, 3, 256, 256), output_dir + 'sample_' +
                str(epoch) + '.png')
    return loss
