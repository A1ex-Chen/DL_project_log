...
def get_parser(description=None):
    ...
    parser.add_argument(
        "-z", "--batch_size", type=int, default=100000, help="batch size"
    )
    parser.add_argument(
        "--step", type=int, default=10000, help="number of rows to inter in each step"
    )
    parser.add_argument(
        "-m", "--model_file", default="saved.model.h5", help="trained model file"
    )
    parser.add_argument(
        "-n",
        "--n_pred",
    ...
    parser.add_argument(
        "--nd", type=int, default=0, help="the first n entries of drugs to subsample"
    )
    parser.add_argument(
        "--use_landmark_genes",
        action="store_true",
    ...
    parser.add_argument(
        "--skip_single_prediction_cleanup",
        action="store_true",
    ...
    parser.add_argument(
        "--pconc_step", type=float, default=1.0, help="concentration step size"
    )
...
def lookup(df, sample, drug1, drug2=None, value=None):
    ...
    df_result = df[
        (df["Sample"] == sample) & (df["Drug1"] == drug1) & (df["Drug2"] == drug2)
    ]
    if df_result.empty:
        df_result = df[
            (df["Sample"] == sample) & (df["Drug1"] == drug2) & (df["Drug2"] == drug1)
        ]
    if value:
        if df_result.empty:
            return 1.0
...
def prepare_data(
    ...
    df_expr = NCI60.load_sample_rnaseq(
...
def main():
    ...
    df_pconc = pd.DataFrame(
        {"pCONC": np.arange(args.min_pconc, args.max_pconc + 0.1, args.pconc_step)}
    )
    ...
    print(
        "Predicting drug response for {} combinations: {} samples x {} drugs x {} drugs x {} doses x {} doses".format(
            n_rows, n_samples, n_drugs, n_drugs, n_doses, n_doses
        )
    )
    ...
    for i in tqdm(range(0, total, args.step)):
        ...
        x_all_list = []
        df_x_all = pd.merge(
            df_all[["Sample"]].iloc[i:j], df_expr, on="Sample", how="left"
        )
        ...
        for k in range(n):
            y_pred = model.predict(
                x_all_list, batch_size=args.batch_size, verbose=0
            ).flatten()
            preds.append(y_pred)
    ...
    if not args.skip_single_prediction_cleanup:
        df_all = df_all[
            (df_all["Drug1"] != df_all["Drug2"])
            | (df_all["pCONC1"] == df_all["pCONC2"])
        ]
        df_sum = df_sum[
            (df_sum["Drug1"] != df_sum["Drug2"])
            | (df_sum["pCONC1"] == df_sum["pCONC2"])
        ]
