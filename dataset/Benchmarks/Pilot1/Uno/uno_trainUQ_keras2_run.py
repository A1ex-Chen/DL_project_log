def run(params):
    ...
    if len(args.gpus) > 0:
        ...
        tf.compat.v1.keras.backend.set_session(tf.compat.v1.Session(config=
            config))
    ...
    if args.export_csv:
        ...
        loader.partition_data(cv_folds=args.cv, train_split=train_split,
            val_split=val_split, cell_types=args.cell_types, by_cell=args.
            by_cell, by_drug=args.by_drug, cell_subset_path=args.
            cell_subset_path, drug_subset_path=args.drug_subset_path,
            exclude_cells=args.exclude_cells, exclude_drugs=args.
            exclude_drugs, exclude_indices=args.exclude_indices)
        train_gen = CombinedDataGenerator(loader, batch_size=args.
            batch_size, shuffle=args.shuffle)
        val_gen = CombinedDataGenerator(loader, partition='val', batch_size
            =args.batch_size, shuffle=args.shuffle)
        x_train_list, y_train = train_gen.get_slice(size=train_gen.size,
            dataframe=True, single=args.single)
        x_val_list, y_val = val_gen.get_slice(size=val_gen.size, dataframe=
            True, single=args.single)
    ...
    if args.export_data:
        ...
        for partition in ['train', 'val']:
            ...
            for i in range(gen.steps):
                x_list, y = gen.get_slice(size=args.batch_size, dataframe=
                    True, single=args.single)
                for j, input_feature in enumerate(x_list):
                    ...
                    store.append('x_{}_{}'.format(partition, j),
                        input_feature.astype('float32'), format='table',
                        data_columns=True)
                store.append('y_{}'.format(partition), y.astype({target:
                    'float32'}), format='table', data_columns=True,
    ...
    if args.use_exported_data is None:
        loader.partition_data(partition_by=args.partition_by, cv_folds=args
            .cv, train_split=train_split, val_split=val_split, cell_types=
            args.cell_types, by_cell=args.by_cell, by_drug=args.by_drug,
            cell_subset_path=args.cell_subset_path, drug_subset_path=args.
            drug_subset_path, exclude_cells=args.exclude_cells,
            exclude_drugs=args.exclude_drugs, exclude_indices=args.
            exclude_indices)
    ...
    def warmup_scheduler(epoch):
        ...
        if epoch <= 5:
            K.set_value(model.optimizer.lr, (base_lr * (5 - epoch) + lr *
                epoch) / 5)
        logger.debug('Epoch {}: lr={:.5g}'.format(epoch, K.get_value(model.
            optimizer.lr)))
    ...
    for fold in range(cv):
        ...
        if args.loss == 'het' or args.loss == 'qtl':
            template_model = candle.add_model_output(template_model, mode=
                args.loss)
        if args.initial_weights:
            logger.info('Loading initial weights from {}'.format(args.
                initial_weights))
        ...
        if len(args.gpus) > 1:
            ...
            model = multi_gpu_model(template_model, cpu_merge=False, gpus=
                gpu_count)
        else:
        ...
        if args.loss == 'het':
            ...
            model.compile(loss=candle.heteroscedastic_loss(nout), optimizer
                =optimizer, metrics=[mae_heteroscedastic,
                r2_heteroscedastic, meanS_heteroscedastic])
        elif args.loss == 'qtl':
            ...
            model.compile(loss=candle.triple_quantile_loss(nout, 0.1, 0.9),
                optimizer=optimizer, metrics=[quantile50, quantile10,
                quantile90])
        else:
            logger.info('Training homoscedastic model:')
            model.compile(loss=args.loss, optimizer=optimizer, metrics=[
                candle.mae, candle.r2])
        ...
        checkpointer = candle.MultiGPUCheckpoint(prefix + cv_ext +
            '.model.h5', save_best_only=True)
        tensorboard = TensorBoard(log_dir='tb/{}{}{}'.format(args.tb_prefix,
            ext, cv_ext))
        ...
        if args.use_exported_data is not None:
            train_gen = DataFeeder(filename=args.use_exported_data,
                batch_size=args.batch_size, shuffle=args.shuffle, single=
                args.single, agg_dose=args.agg_dose)
            val_gen = DataFeeder(partition='val', filename=args.
                use_exported_data, batch_size=args.batch_size, shuffle=args
                .shuffle, single=args.single, agg_dose=args.agg_dose)
            test_gen = DataFeeder(partition='test', filename=args.
                use_exported_data, batch_size=args.batch_size, shuffle=args
                .shuffle, single=args.single, agg_dose=args.agg_dose)
        else:
            train_gen = CombinedDataGenerator(loader, fold=fold, batch_size
                =args.batch_size, shuffle=args.shuffle, single=args.single)
            val_gen = CombinedDataGenerator(loader, partition='val', fold=
                fold, batch_size=args.batch_size, shuffle=args.shuffle,
                single=args.single)
            test_gen = CombinedDataGenerator(loader, partition='test', fold
                =fold, batch_size=args.batch_size, shuffle=args.shuffle,
                single=args.single)
        ...
        if args.no_gen:
            x_train_list, y_train = train_gen.get_slice(size=train_gen.size,
                single=args.single)
            x_val_list, y_val = val_gen.get_slice(size=val_gen.size, single
                =args.single)
            history = model.fit(x_train_list, y_train, batch_size=args.
                batch_size, epochs=args.epochs, callbacks=callbacks,
                validation_data=(x_val_list, y_val))
        else:
            logger.info(
                'Data points per epoch: train = %d, val = %d, test = %d',
                train_gen.size, val_gen.size, test_gen.size)
            logger.info('Steps per epoch: train = %d, val = %d, test = %d',
                train_gen.steps, val_gen.steps, test_gen.steps)
            history = model.fit(train_gen, epochs=args.epochs, callbacks=
                callbacks, validation_data=val_gen)
        if test_gen.size > 0:
            ...
            if args.loss == 'het':
                ...
                y_val_pred = y_val_pred_50q.flatten()
        else:
            if args.no_gen:
                y_val_pred = model.predict(x_val_list, batch_size=args.
                    batch_size)
            else:
                val_gen.reset()
        ...
        elif 'r2' in history.history.keys():
    ...
    if args.agg_dose:
        if args.single:
            ...
            df_pred.sort_values(['Source', 'Sample', 'Drug1', 'Drug2',
                target], inplace=True)
    elif args.single:
    ...
    if args.cp:
        ...
        logger.info('Model weights stored in file: {}'.format(args.
            save_path + '/' + args.save_weights))
    if args.cv > 1:
        scores = evaluate_prediction(df_pred[target], df_pred['Predicted' +
            target])
        log_evaluation(scores, description='Combining cross validation folds:')
    for test_source in loader.test_sep_sources:
        test_gen = CombinedDataGenerator(loader, partition='test',
            batch_size=args.batch_size, source=test_source)
        ...
        if args.no_gen:
            ...
            y_test_pred = model.predict_generator(test_gen.flow(single=args
                .single), test_gen.steps)
            if args.loss == 'het':
        ...
        log_evaluation(scores, description='Testing on data from {} ({})'.
            format(test_source, n_test))
