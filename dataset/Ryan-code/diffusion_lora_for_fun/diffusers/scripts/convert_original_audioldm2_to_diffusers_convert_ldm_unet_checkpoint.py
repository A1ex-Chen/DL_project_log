def convert_ldm_unet_checkpoint(checkpoint, config, path=None, extract_ema=
    ...
    for i in range(1, num_input_blocks):
        ...
        if len(attentions):
            ...
            meta_path = [{'old': f'input_blocks.{i}.{1 + layer_id}', 'new':
                f'down_blocks.{block_id}.attentions.{layer_in_block_id * num_attention_layers + layer_id}'
    ...
    meta_path = {'old': f'middle_block.{len(middle_blocks) - 1}', 'new':
        'mid_block.resnets.1'}
    ...
    for i in range(1, num_middle_blocks - 1):
        ...
        meta_path = {'old': f'middle_block.{i}', 'new':
            f'mid_block.attentions.{i - 1}'}
        assign_to_checkpoint(attentions_paths, new_checkpoint,
    ...
    for i in range(num_output_blocks):
        ...
        if len(output_block_list) > 1:
            ...
            attentions = [key for key in output_blocks[i] if 
                f'output_blocks.{i}.0' not in key]
            paths = renew_resnet_paths(resnets)
            ...
            if len(attentions):
                paths = renew_attention_paths(attentions)
                meta_path = [{'old': f'output_blocks.{i}.{1 + layer_id}',
                    'new':
                    f'up_blocks.{block_id}.attentions.{layer_in_block_id * num_attention_layers + layer_id}'
                    } for layer_id in range(num_attention_layers)]
                assign_to_checkpoint(paths, new_checkpoint, unet_state_dict,
                    additional_replacements=meta_path, config=config)
        else:
