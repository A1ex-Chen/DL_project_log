def create_ip_adapter_faceid_state_dict(model):
    ...
    image_projection = IPAdapterFaceIDImageProjection(cross_attention_dim=
        cross_attention_dim, image_embed_dim=cross_attention_dim, mult=2,
        num_tokens=4)
    ip_image_projection_state_dict = {}
    sd = image_projection.state_dict()
    ip_image_projection_state_dict.update({'proj.0.weight': sd[
        'ff.net.0.proj.weight'], 'proj.0.bias': sd['ff.net.0.proj.bias'],
        'proj.2.weight': sd['ff.net.2.weight'], 'proj.2.bias': sd[
        'ff.net.2.bias'], 'norm.weight': sd['norm.weight'], 'norm.bias': sd
        ['norm.bias']})
    del sd
    ip_state_dict = {}
