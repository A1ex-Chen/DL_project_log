def get_dummy_components(self):
    ...
    unet = UNet3DConditionModel(block_out_channels=(4, 8), layers_per_block
        ...
        'UpBlock3D', 'CrossAttnUpBlock3D'), cross_attention_dim=32,
        attention_head_dim=4, norm_num_groups=2)
    scheduler = DDIMScheduler(beta_start=0.00085, beta_end=0.012,
        beta_schedule='scaled_linear', clip_sample=True, set_alpha_to_one=False
        )
    torch.manual_seed(0)
    vae = AutoencoderKL(block_out_channels=[8], in_channels=3, out_channels
        =3, down_block_types=['DownEncoderBlock2D'], up_block_types=[
        'UpDecoderBlock2D'], latent_channels=4, sample_size=32,
        norm_num_groups=2)
    torch.manual_seed(0)
