@torch.no_grad()
def __call__(self, prompt: Union[str, List[str]]=None, ref_image: Union[
    torch.Tensor, PIL.Image.Image]=None, height: Optional[int]=None, width:
    ...
    float=0.0, attention_auto_machine_weight: float=1.0,
    ...
    def hacked_UpBlock2D_forward(self, hidden_states: torch.Tensor,
        res_hidden_states_tuple: Tuple[torch.Tensor, ...], temb: Optional[
        torch.Tensor]=None, upsample_size: Optional[int]=None, **kwargs: Any
        ) ->torch.Tensor:
        eps = 1e-06
        for i, resnet in enumerate(self.resnets):
            res_hidden_states = res_hidden_states_tuple[-1]
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            ref_xt = torch.cat([ref_xt] * 2
                ) if do_classifier_free_guidance else ref_xt
