...
def __init__(self, num_train_timesteps: int=1000, beta_start: float=0.0001,
    ...
    solver_type: str='midpoint', lower_order_final: bool=False,
    use_karras_sigmas: Optional[bool]=False, final_sigmas_type: Optional[
    str]='zero', lambda_min_clipped: float=-float('inf'), variance_type:
    Optional[str]=None):
    if algorithm_type == 'dpmsolver':
        deprecation_message = (
            'algorithm_type `dpmsolver` is deprecated and will be removed in a future version. Choose from `dpmsolver++` or `sde-dpmsolver++` instead'
            )
