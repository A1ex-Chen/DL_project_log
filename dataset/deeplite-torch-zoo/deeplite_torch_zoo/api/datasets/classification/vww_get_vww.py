...
def get_vww(data_root=None, img_size=224, batch_size=64, test_batch_size=
    None, use_prefetcher=False, num_workers=1, distributed=False,
    ...
    dataset_train = VisualWakeWordsClassification(root=os.path.join(
        data_root, 'all'), annFile=os.path.join(data_root,
        'annotations/instances_train.json'), transform=train_transforms or
        default_train_transforms)
    dataset_eval = VisualWakeWordsClassification(root=os.path.join(
        data_root, 'all'), annFile=os.path.join(data_root,
        'annotations/instances_val.json'), transform=val_transforms or
