def to_mot_eval_format(gt_bbdf: BBoxDataFrame, pred_bbdf: BBoxDataFrame
    ) ->dict[str, Any]:
    ...
    if gt_bbdf.size == 0 and pred_bbdf.size == 0:
        ...
        return data
    min_frame = min(gt_bbdf.first_valid_index() or pred_bbdf.
        first_valid_index(), pred_bbdf.first_valid_index() or gt_bbdf.
        first_valid_index())
    max_frame = max(gt_bbdf.last_valid_index() or pred_bbdf.
        last_valid_index(), pred_bbdf.last_valid_index() or gt_bbdf.
        last_valid_index())
    pred_bbdf = pred_bbdf.reindex(range(min_frame, max_frame + 1))
    gt_bbdf = gt_bbdf.reindex(range(min_frame, max_frame + 1))
    assert pred_bbdf.index.equals(gt_bbdf.index
    ...
    if num_gt_dets == 0:
        ...
        data['num_timesteps'] = 0
        return data
    tracker_dets_xyxy = [[convert_to_x1y1x2y2(bbox) for bbox in frame_dets] for
        frame_dets in pred_dets]
    gt_dets_xyxy = [[convert_to_x1y1x2y2(bbox) for bbox in frame_dets] for
        frame_dets in gt_dets]
    sim_score_list = []
