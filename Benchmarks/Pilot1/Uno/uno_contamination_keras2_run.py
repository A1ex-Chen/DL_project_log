def run(params):
    ...
    for fold in range(cv):
        ...
        es_monitor = keras.callbacks.EarlyStopping(monitor=
            'val_mae_contamination', patience=10, verbose=1)
        reduce_lr = ReduceLROnPlateau(monitor='val_mae_contamination',
            factor=0.5, patience=5, min_lr=1e-05)
        ...
        contamination_cbk = candle.Contamination_Callback(x_train_list,
            y_train, args.a_max)
        model.compile(loss=candle.contamination_loss(nout,
            contamination_cbk.T_k, contamination_cbk.a, contamination_cbk.
            sigmaSQ, contamination_cbk.gammaSQ), optimizer=optimizer,
            metrics=[candle.mae_contamination_metric(nout), candle.
            r2_contamination_metric(nout)])
        ...
        history = model.fit(x_train_list, y_train_augmented, batch_size=
            args.batch_size, epochs=args.epochs, callbacks=callbacks,
        ...
        title = 'a Evolution'
        candle.plot_array(contamination_cbk.avalues, xlabel, ylabel, title,
            fname)
        fname = prefix + '.evol.sigmasq.png'
        xlabel = 'Epochs'
        ylabel = 'Contamination SigmaSQ'
        title = 'SigmaSQ Evolution'
        candle.plot_array(contamination_cbk.sigmaSQvalues, xlabel, ylabel,
            title, fname)
        fname = prefix + '.evol.gammasq.png'
        xlabel = 'Epochs'
        ylabel = 'Contamination GammaSQ'
        title = 'GammaSQ Evolution'
        candle.plot_array(contamination_cbk.gammaSQvalues, xlabel, ylabel,
            title, fname)
        ...
        candle.plot_contamination(y_train, y_tr_pred.squeeze(), sigma, T,
            pred_name=target, figprefix=prefix)
    ...
    for test_source in loader.test_sep_sources:
        ...
        if n_test == 0:
            continue
        x_test_list, y_test = test_gen.get_slice(size=test_gen.size, single
            =args.single)
