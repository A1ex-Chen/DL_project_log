def run(params):
    ...
    for fold in range(cv):
        ...
        model.compile(loss=args.loss, optimizer=optimizer, metrics=[candle.
            mae, candle.r2])
        ...
        if args.use_exported_data is not None:
            train_gen = DataFeeder(filename=args.use_exported_data,
                ...
                args.single, agg_dose=args.agg_dose, on_memory=args.
                on_memory_loader)
        ...
        candle.plot_metrics(history, title=None, skip_ep=0, outdir=os.path.
            dirname(args.save_path), add_lr=True)
    ...
    if args.cv > 1:
        ...
        log_evaluation(scores, logger, description=
            'Combining cross validation folds:')
