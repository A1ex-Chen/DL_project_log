def run(gParameters):
    ...
    data_loc = candle.fetch_file(origin + train_data, unpack=True, md5_hash
        =None, subdir='Pilot3')
    ...
    for k in range(n_layers):
        ...
        if k == 0:
            model.add(LSTM(rnn_size, input_shape=(maxlen, len(chars)),
                return_sequences=ret_seq, dropout=dropout,
                recurrent_dropout=recurrent_dropout))
        else:
            model.add(LSTM(rnn_size, dropout=dropout, recurrent_dropout=
                recurrent_dropout, return_sequences=ret_seq))
    ...
    for iteration in range(1, n_epochs + 1):
        ...
        with open(dirname + '/model_' + str(iteration) + '_' + '{:f}'.
            format(loss) + '.json', 'w') as json_file:
        ...
        if do_sample:
            outtext = open(dirname + '/example_' + str(iteration) + '_' +
                '{:f}'.format(loss) + '.txt', 'w', encoding='utf-8')
