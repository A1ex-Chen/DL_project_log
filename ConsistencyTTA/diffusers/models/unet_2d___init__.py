...
def __init__(self, sample_size: Optional[Union[int, Tuple[int, int]]]=None,
    ...
    flip_sin_to_cos: bool=True, down_block_types: Tuple[str]=('DownBlock2D',
    'AttnDownBlock2D', 'AttnDownBlock2D', 'AttnDownBlock2D'),
    up_block_types: Tuple[str]=('AttnUpBlock2D', 'AttnUpBlock2D',
    'AttnUpBlock2D', 'UpBlock2D'), block_out_channels: Tuple[int]=(224, 448,
    672, 896), layers_per_block: int=2, mid_block_scale_factor: float=1,
    downsample_padding: int=1, act_fn: str='silu', attention_head_dim:
    Optional[int]=8, norm_num_groups: int=32, norm_eps: float=1e-05,
    resnet_time_scale_shift: str='default', add_attention: bool=True,
    class_embed_type: Optional[str]=None, num_class_embeds: Optional[int]=None
    ):
    super().__init__()
    ...
    for i, down_block_type in enumerate(down_block_types):
        ...
        down_block = get_down_block(down_block_type, num_layers=
            ...
            resnet_groups=norm_num_groups, attn_num_head_channels=
            attention_head_dim, downsample_padding=downsample_padding,
    ...
    self.mid_block = UNetMidBlock2D(in_channels=block_out_channels[-1],
        ...
        norm_num_groups, add_attention=add_attention)
