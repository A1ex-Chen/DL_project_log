def get_audio_features(sample, audio_data, max_len, data_truncating,
    data_filling, audio_cfg, require_grad=False):
    """
            ...
            if data_truncating == 'rand_trunc':
                ...
                if chunk_frames == total_frames:
                    ...
                    mel_shrink = torchvision.transforms.Resize(size=[
                        chunk_frames, audio_cfg['mel_bins']])(mel[None])[0]
                    mel_fusion = torch.stack([mel_shrink, mel_chunk_front,
                        mel_chunk_middle, mel_chunk_back], dim=0)
                    sample['mel_fusion'] = mel_fusion
                    longer = torch.tensor([True])
            else:
