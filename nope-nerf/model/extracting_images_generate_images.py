def generate_images(self, data, render_dir, c2ws, fxfy, it, output_geo):
    ...
    with torch.no_grad():
        rgb_pred = []
        depth_pred = []
        for ii, (pixels_i, depth_i) in enumerate(zip(torch.split(pixels,
    ...
    if output_geo:
        with torch.no_grad():
            ...
            rgb_pred = [self.renderer(pixels_i, None, camera_mat, world_mat,
                scale_mat, 'phong_renderer', eval_=True, it=it, add_noise=
                False)['rgb'] for ii, pixels_i in enumerate(torch.split(
                pixels, 1024, dim=1))]
        ...
        imageio.imwrite(os.path.join(geo_out_dir, str(img_idx).zfill(4) +
            '.png'), geo_out)
    else:
        geo_out = None
