def train(cfg):
    ...
    train_loader, train_dataset = dl.get_dataloader(cfg, mode=mode, shuffle
        =cfg['dataloading']['shuffle'])
    test_loader, _ = dl.get_dataloader(cfg, mode=mode, shuffle=cfg[
        'dataloading']['shuffle'])
    ...
    optimizer = optim.Adam(nope_nerf.parameters(), lr=lr, weight_decay=
        weight_decay)
    checkpoint_io = mdl.CheckpointIO(out_dir, model=nope_nerf, optimizer=
        optimizer)
    load_dir = cfg['training']['load_dir']
    try:
        load_dict = checkpoint_io.load(load_dir, load_model_only=cfg[
            'training']['load_ckpt_model_only'])
    ...
    scheduling_start = load_dict.get('scheduling_start', cfg['training'][
        'scheduling_start'])
    if not auto_scheduler:
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=
            list(range(scheduling_start, scheduling_epoch +
            scheduling_start, 10)), gamma=cfg['training']['scheduler_gamma'
            ], last_epoch=epoch_it)
    if cfg['pose']['learn_pose']:
        if cfg['pose']['init_pose']:
            ...
            init_pose = None
        pose_param_net = mdl.LearnPose(n_views, cfg['pose']['learn_R'], cfg
            ['pose']['learn_t'], cfg, init_c2w=init_pose).to(device=device)
        optimizer_pose = optim.Adam(pose_param_net.parameters(), lr=cfg[
            'training']['pose_lr'])
        ...
        try:
            ...
            load_dict = checkpoint_io_pose.load(pose_load_dir,
        ...
        if not auto_scheduler:
            scheduler_pose = torch.optim.lr_scheduler.MultiStepLR(
                optimizer_pose, milestones=list(range(scheduling_start, 
                scheduling_epoch + scheduling_start, 100)), gamma=cfg[
                'training']['scheduler_gamma_pose'], last_epoch=epoch_it)
    else:
        optimizer_pose = None
        pose_param_net = None
    if cfg['distortion']['learn_distortion']:
        distortion_net = mdl.Learn_Distortion(n_views, cfg['distortion'][
            'learn_scale'], cfg['distortion']['learn_shift'], cfg).to(device
            =device)
        optimizer_distortion = optim.Adam(distortion_net.parameters(), lr=
            cfg['training']['distortion_lr'])
        checkpoint_io_distortion = mdl.CheckpointIO(out_dir, model=
            distortion_net, optimizer=optimizer_distortion)
        try:
        ...
        if not auto_scheduler:
            scheduler_distortion = torch.optim.lr_scheduler.MultiStepLR(
                optimizer_distortion, milestones=list(range(
                scheduling_start, scheduling_epoch + scheduling_start, 100)
    ...
    if cfg['pose']['learn_focal']:
        if cfg['pose']['init_focal_type'] == 'gt':
            init_focal = [train_dataset['img'].K[0, 0], -train_dataset[
                'img'].K[1, 1]]
        else:
            init_focal = None
        focal_net = mdl.LearnFocal(cfg['pose']['update_focal'], cfg['pose']
            ['fx_only'], order=cfg['pose']['focal_order'], init_focal=
            init_focal).to(device=device)
        optimizer_focal = torch.optim.Adam(focal_net.parameters(), lr=cfg[
            'training']['focal_lr'])
        ...
        if not auto_scheduler:
            scheduler_focal = torch.optim.lr_scheduler.MultiStepLR(
                optimizer_focal, milestones=list(range(scheduling_start, 
                scheduling_epoch + scheduling_start, 100)), gamma=cfg[
                'training']['scheduler_gamma_focal'], last_epoch=epoch_it)
    else:
        optimizer_focal = None
        focal_net = None
    training_cfg = cfg['training']
    trainer = mdl.Trainer(nope_nerf, optimizer, training_cfg, device=device,
        optimizer_pose=optimizer_pose, pose_param_net=pose_param_net,
        optimizer_focal=optimizer_focal, focal_net=focal_net,
        optimizer_distortion=optimizer_distortion, distortion_net=
        distortion_net, cfg_all=cfg)
    ...
    while epoch_it < scheduling_start + scheduling_epoch:
        ...
        for batch in train_loader:
            ...
            loss_dict = trainer.train_step(batch, it, epoch_it,
                scheduling_start, render_path)
            loss = loss_dict['loss']
            ...
            if visualize_every > 0 and it % visualize_every == 0:
                ...
                val_rgb = trainer.render_visdata(data_test, cfg['training']
                    ['vis_resolution'], it, out_render_path)
            ...
            if checkpoint_every > 0 and it % checkpoint_every == 0:
                ...
                checkpoint_io.save('model.pt', epoch_it=epoch_it, it=it,
                    loss_val_best=metric_val_best, scheduling_start=
                    scheduling_start, patient_count=patient_count)
                if cfg['pose']['learn_pose']:
                    checkpoint_io_pose.save('model_pose.pt', epoch_it=
                        epoch_it, it=it)
                if cfg['pose']['learn_focal']:
                    checkpoint_io_focal.save('model_focal.pt', epoch_it=
                        epoch_it, it=it)
                if cfg['distortion']['learn_distortion']:
                    checkpoint_io_distortion.save('model_distortion.pt',
                        epoch_it=epoch_it, it=it)
            if backup_every > 0 and it % backup_every == 0:
                logger_py.info('Backup checkpoint')
                checkpoint_io.save('model_%d.pt' % it, epoch_it=epoch_it,
                    it=it, loss_val_best=metric_val_best, scheduling_start=
                    scheduling_start, patient_count=patient_count)
                if cfg['pose']['learn_pose']:
                    checkpoint_io_pose.save('model_pose_%d.pt' % it,
                        epoch_it=epoch_it, it=it)
                if cfg['pose']['learn_focal']:
                    checkpoint_io_focal.save('model_focal_%d.pt' % it,
                        epoch_it=epoch_it, it=it)
                if cfg['distortion']['learn_distortion']:
                    checkpoint_io_distortion.save('model_distortion_%d.pt' %
                        it, epoch_it=epoch_it, it=it)
        ...
        if eval_pose_every > 0 and epoch_it % eval_pose_every == 0:
            with torch.no_grad():
                learned_poses = torch.stack([pose_param_net(i) for i in
                    range(n_views)])
            c2ws_est_aligned = align_ate_c2b_use_a2b(learned_poses, gt_poses)
            ate = compute_ATE(gt_poses.cpu().numpy(), c2ws_est_aligned.cpu(
                ).numpy())
            rpe_trans, rpe_rot = compute_rpe(gt_poses.cpu().numpy(),
                c2ws_est_aligned.cpu().numpy())
            tqdm.write('{0:6d} ep: Train: ATE: {1:.3f} RPE_r: {2:.3f}'.
                format(epoch_it, ate, rpe_rot * 180 / np.pi))
            eval_dict = {'ate_trans': ate, 'rpe_trans': rpe_trans * 100,
        ...
        if not auto_scheduler:
            ...
            if epoch_it < scheduling_start:
                ...
                new_lr = cfg['training']['learning_rate'] * cfg['training'][
                    'scheduler_gamma'] ** int((epoch_it - scheduling_start) /
                    10)
                for param_group in optimizer.param_groups:
                    param_group['lr'] = new_lr
                if cfg['pose']['learn_pose']:
                    new_lr_pose = cfg['training']['pose_lr'] * cfg['training'][
                        'scheduler_gamma_pose'] ** int((epoch_it -
                        scheduling_start) / 100)
                    for param_group in optimizer_pose.param_groups:
                        param_group['lr'] = new_lr_pose
                if cfg['pose']['learn_focal']:
                    new_lr_focal = cfg['training']['focal_lr'] * cfg['training'
                        ]['scheduler_gamma_focal'] ** int((epoch_it -
                        scheduling_start) / 100)
                    for param_group in optimizer_focal.param_groups:
                        param_group['lr'] = new_lr_focal
                if cfg['distortion']['learn_distortion']:
                    new_lr_distortion = cfg['training']['distortion_lr'] * cfg[
                        'training']['scheduler_gamma_distortion'] ** int((
