def eval(cfg):
    ...
    train_loader, train_dataset = get_dataloader(cfg, mode='train', shuffle
        =False)
    ...
    if use_learnt_focal:
        focal_net = mdl.LearnFocal(cfg['pose']['learn_focal'], cfg['pose'][
            'fx_only'], order=cfg['pose']['focal_order'])
        checkpoint_io_focal = mdl.CheckpointIO(out_dir, model=focal_net)
        checkpoint_io_focal.load(cfg['extract_images']['model_file_focal'])
        fxfy = focal_net(0)
        print('learned fx: {0:.2f}, fy: {1:.2f}'.format(fxfy[0].item(),
            fxfy[1].item()))
    else:
        focal_net = None
        fxfy = None
    if use_learnt_poses:
        if cfg['pose']['init_pose']:
            ...
            init_pose = None
        learned_pose_param_net = mdl.LearnPose(train_dataset['img'].N_imgs,
            cfg['pose']['learn_R'], cfg['pose']['learn_t'], cfg=cfg,
            init_c2w=init_pose).to(device=device)
        checkpoint_io_pose = mdl.CheckpointIO(out_dir, model=
            learned_pose_param_net)
        ...
        if cfg['eval_pose']['type_to_eval'] == 'train':
            ...
            with torch.no_grad():
                ...
                learned_c2ws_train = torch.stack([learned_pose_param_net(i) for
                    i in range(train_dataset['img'].N_imgs_train)])
                colmap_c2ws_train = train_dataset['img'].c2ws
                ...
                if init_method == 'scale':
                    ...
                    init_c2ws = align_ate_c2b_use_a2b(colmap_c2ws_train,
                        learned_c2ws_train, init_c2ws)
                elif init_method == 'pre':
                    sample_rate = train_dataset['img'].sample_rate
                    init_c2ws = learned_c2ws_train[int(sample_rate / 2) - 1
                        ::sample_rate - 1][:N_imgs]
                elif init_method == 'none':
                    init_c2ws = None
            eval_pose_param_net = mdl.LearnPose(eval_dataset['img'].N_imgs,
                learn_R=True, learn_t=True, cfg=cfg, init_c2w=init_c2ws).to(
                device=device)
        optimizer_eval_pose = torch.optim.Adam(eval_pose_param_net.
            parameters(), lr=opt_eval_lr)
        scheduler_eval_pose = torch.optim.lr_scheduler.MultiStepLR(
            optimizer_eval_pose, milestones=list(range(0, int(num_epoch),
            int(num_epoch / 5))), gamma=0.5)
        """Optimise eval poses"""
        if cfg['eval_pose']['type_to_eval'] != 'train':
            eval_pose_cfg = cfg['eval_pose']
            trainer = mdl.Trainer_pose(nope_nerf, eval_pose_cfg, device=
                device, optimizer_pose=optimizer_eval_pose, pose_param_net=
                eval_pose_param_net, focal_net=focal_net)
            for epoch_i in tqdm(range(num_epoch), desc='optimising eval'):
                ...
                tqdm.write('{0:6d} ep: Opt: L2 loss: {1:.4f}, PSNR: {2:.3f}'
                    .format(epoch_i, L2_loss_mean, opt_pose_psnr))
    eval_pose_param_net.eval()
    eval_c2ws = torch.stack([eval_pose_param_net(i) for i in range(N_imgs)])
    generator = Eval_Images(renderer, cfg, use_learnt_poses=
        use_learnt_poses, use_learnt_focal=use_learnt_focal, device=device,
        render_type=cfg['rendering']['type'], c2ws=eval_c2ws, img_list=img_list
        )
    ...
    max_depth = 20
    for data in loader:
        out = generator.eval_images(data, render_dir, fxfy, lpips_metric,
            logger=logger, min_depth=min_depth, max_depth=max_depth)
        imgs.append(out['img'])
    ...
    print('--------------------------')
    print('Mean MSE: {0:.2f}, PSNR: {1:.2f}, SSIM: {2:.2f}, LPIPS {3:.2f}'.
        format(mean_mse, mean_psnr, mean_ssim, mean_lpips))
    print('{0:.2f}'.format(mean_psnr), '&{0:.2f}'.format(mean_ssim), '&',
        '{0:.2f}'.format(mean_lpips))
    if cfg['extract_images']['eval_depth']:
        depth_errors = []
        ratio = np.median(np.concatenate(depth_gts)) / np.median(np.
            concatenate(depth_preds))
        ...
        print('\n-> Done!')
        with open(os.path.join(generation_dir, 'depth_evaluation.txt'), 'a'
            ) as f:
            f.writelines(('{:>8} | ' * 7).format('abs_rel', 'sq_rel',
    ...
    imageio.mimwrite(os.path.join(video_out_dir, 'img.mp4'), imgs, fps=30,
        quality=9)
